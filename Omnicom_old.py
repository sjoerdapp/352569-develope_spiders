...
class QuotesInfiniteScrollSpider(scrapy.Spider):
    ...
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://sp5001/Omnicom_1367000ARV001/',
        }
    ...
    def parse(self, response):  # follow drop down menue for different years
        auxs = response.xpath('//ul[@class="headlines list"]/li')
        for aux in auxs:
            item = SwisscomIvCrawlerItem()
            item['PUBSTRING'] = aux.xpath('./div/text()').extract_first()
        ...
        years = list(range(0, 10)) # fill in years which should be scraped, always last yeat +1 as upper bound will not be element of the list
        #del years[0]  # delets first element "NULL" from list of years
        for year in years:
            aux_url = 'http://omnicom.q4web.com/feed/PressRelease.svc/GetPressReleaseList?CategoryId=1cb807d2-208f-4bc3-9133-6a9ad45ac3b0&apiKey=B5595FEB4174416297055DC1E4CE1676&callback=angular.callbacks._0&includeTags=true&pageNumber={}&pageSize=10&pressReleaseDateFilter=1'
    ...
    def parse_next(self, response):
        body = response.body
        dats = json.loads(body.decode("utf-8").split('backs._0(')[1].split(');')[0])
        for dat in dats['GetPressReleaseListResult']:
            ...
            #        }
            base_url = 'http://www.omnicomgroup.com/newsroom/article?newsId='
