...
### CBRE Group Inc 1|2
### 1st spider Investor Press Releases, 2nd spider Media Center
### normal get 
### back to 20030213


class QuotessSpider(scrapy.Spider):
    name = 'CBRE_I_4527000ARV001'
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/CBRE_I_4527000ARV001/',
        }
    #custom_settings = {
    ...
    def parse(self, response):  # follow drop down menue for different years
         ...
         for year in years:
             aux_url = 'https://ir.cbre.com/press-releases/{}/default.aspx'
             year_url = [aux_url.format(year)][0]
             yield scrapy.Request(url=year_url, callback=self.parse_next)

    def parse_next(self, response):
          auxs = response.xpath('//div[contains(@class, "module_container--content")]/div[@class="module_item"]')
          for aux in auxs:
              ...
              aux_url = aux.xpath('./div[@class="module_headline"]/a/@href').extract_first()
    ...
    def parse_details(self, response):
        ...
        name_regex = r'(Forward(.|\s*)Looking\s*Statements)(.|\s)*|(\bABOUT\s*CBRE\s*Group\b)(.|\s)*|(\bABOUT.CBRE\s*Group\b)(.|\s)*' 
