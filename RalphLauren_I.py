...
class QuotessSpider(scrapy.Spider):
    name = 'RalphLauren_I_3200900ARV001'
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/RalphLauren_I_3200900ARV001/',
        }
    #custom_settings = {
    ...
    #}
    start_urls = ['http://investor.ralphlauren.com/phoenix.zhtml?c=65933&p=irol-news']
    ...
    def parse(self, response):
          auxs = response.xpath('//table[@class="ccbnNewsList"]//tr[not(contains(@class, "Ttl"))][not(contains(@class, "Txt"))]')
          for aux in auxs:
    ...
    def parse_details(self, response):
        ...
        if '.pdf' in response.url.lower() or 'external.file' in response.url.lower():
            ...
            item['DESCRIPTION'] = re.sub(name_regex,'' ," ".join(response.xpath('//span[@class="ccbnTxt ccbnNwsA"]//text()[not(ancestor::div[@class="box__right"] or self::style or self::script or  ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE)
