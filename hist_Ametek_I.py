...
### Ametek Inc 1|2
### 1 IR financial releases, 2nd spider Product and business news;
### normal get
### history back to 20140103


class QuotessSpider(scrapy.Spider):
    name = 'Ametek_I_3205400ARV001'
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/Ametek_I_3205400ARV001/',
        }
    #custom_settings = {
    ...
    #}
    start_urls = ['https://investors.ametek.com/news-releases?9de708dd_year%5Bvalue%5D=_none&op=Filter&9de708dd_widget_id=9de708dd&form_build_id=form-R6NmpyEBfO7uu8gbtkBCwQyseam-s5E_Rs9gcWpBOds&form_id=widget_form_base']
    ...
    #     for year in years:
    #         aux_url = 'https://ir.wabteccorp.com/investor-relations/financial-releases?page={}'
    #         year_url = [aux_url.format(year)][0]
    #         yield scrapy.Request(url=year_url, callback=self.parse_next)

    def parse(self, response):
          auxs = response.xpath('//table//tr[@class="ccbnBgTblOdd"]')
          for aux in auxs[0:20]:
              ...
              #        }
              base_url = 'https://investors.ametek.com'
    ...
    def parse_details(self, response):
        ...
        name_regex = r'(Forward(.|\s*)Looking\s*Statements)(.|\s)*|(AMETEK.\s*Inc..?\s*(\(\s*www.wabtec.com\s*\)\s*)?is\s*a\s*leading\s*global\s*provider)(.|\s)*'
