...
### Packaging Corporation of America 1|2
### 1st spider IR News Releases, 2nd News Room
### normal get,  
### back to 20010416


class QuotessSpider(scrapy.Spider):
    name = 'PackagingCorporation_I_2351100ARV001'
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/PackagingCorporation_I_2351100ARV001/',
        }
    #custom_settings = {
    ...
    #}
    start_urls = ['http://ir.packagingcorp.com']
    ...
    def parse_next(self, response):
          ...
          for aux in auxs:
              ...
              item['HEADLINE']= aux.xpath('.//a[not(contains(@class, "summary"))]/text()').extract_first()
              item['DOCLINK']= aux.xpath('.//a[not(contains(@class, "summary"))]/@href').extract_first()
              #item = {
              #        'PUBSTRING': aux.xpath('./p[@class="news-card-date"]//text()').extract()[1],
              #        'HEADLINE': aux.xpath('.//h3[@class="news-card-title"]/a//text()').extract_first(),
              #        'DOCLINK': aux.xpath('.//h3[@class="news-card-title"]/a/@href').extract_first(),
              #        }
              base_url = 'http://ir.packagingcorp.com'
