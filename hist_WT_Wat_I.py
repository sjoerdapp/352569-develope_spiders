...
### Willis Towers Watson Public Limited Company 1|2
### 1st spider Investor section 2nd spider Press
### Invester section spide classic page 
### 20160105

class QuotesInfiniteScrollSpider(scrapy.Spider):
    name = "WT_wat_I_9900062ARV001"
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/WT_wat_I_9900062ARV001/',
        }
    ...
    start_urls = ['http://investors.willistowerswatson.com/news-releases?field_nir_news_date_value%5Bmin%5D=2018#views-exposed-form-widget-news-widget-news-ul'] 
    ...
    def parse(self, response):  # follow drop down menue for different years
         ...
         for year in years:
             aux_url = 'http://investors.willistowerswatson.com/news-releases?field_nir_news_date_value%5Bmin%5D={}#views-exposed-form-widget-news-widget-news-ul'
    ...
    def parse_next(self, response):
        auxs = response.xpath('//div[@class="item-list"]/table//tr[not(self::*[@class="ccbnBgTblTtl"])]')
        for aux in auxs:
            item = SwisscomIvCrawlerItem()
            item['PUBSTRING'] = aux.xpath('./td//div[@class="field__item"]/text()').extract_first()
            item['HEADLINE']= aux.xpath('./td/a[2]/text()').extract_first()
            item['DOCLINK']= aux.xpath('./td/a[2]/@href').extract_first()
            #item = {
            ...
            #        }
            base_url = 'http://investors.willistowerswatson.com'
            aux_url = aux.xpath('./td/a[2]/@href').extract_first()
    ...
    def parse_details(self, response):
        ...
        name_regex = r'(Forward(.|\s*)Looking\s*Statements)(.|\s)*|(\bAbout\s*Willis\s*Towers\s*Watson\b)(.|\s)* | (\bAbout.Willis.Towers.Watson\b)(.|\s)* | (\bAbout.Willis.Re\b)(.|\s)*'
