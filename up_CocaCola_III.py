...
class QuotessSpider(scrapy.Spider):
    name = 'CocaCola_III_2046800ARV003'
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/CocaCola_III_2046800ARV003/',
        }
    #custom_settings = {
    ...
    #}
    start_urls = ['https://3adnjyji1d.execute-api.us-east-1.amazonaws.com/prod01-journey/search?format=sdk&pretty=true&fq=%28and%20site%3A%27tccc-us-en%27%20language%3A%20%27en%27%20%28phrase%20field%3Dcontent_type%27article%27%29%20%29&q=%27us%27&q.parser=structured&size=50&sort=published_date%20desc']
    ...
    def parse(self, response):
          ...
          for aux in body['hits']['hit'][0:20]:
              ...
              item['DESCRIPTION'] = aux['fields']['body']
              #item = {
    ...
    def parse_details(self, response):
        ...
        if '.pdf' in response.url.lower() or 'external.file' in response.url.lower():
          ...
          if not re.search('[a-zA-Z]', item['DESCRIPTION']):
            item['DESCRIPTION'] = re.sub(name_regex,'' ," ".join(response.xpath('//div[@class="cq-dd-fragment pt-3 pt-lg-5"]//text()[not(ancestor::div[@class="box__right"] or self::style or self::script or ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE)
            item['DOCLINK'] = response.url
            if not re.search('[a-zA-Z]', item['DESCRIPTION']):
                ...
                if not re.search('[a-zA-Z]', item['DESCRIPTION']):
                    item['DESCRIPTION'] = re.sub(name_regex,'' ," ".join(response.xpath('//div[@class="text__container"]//text()[not(ancestor::div[@class="box__right"] or self::style or self::script or ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE)
                    yield item
                else:
                    yield item
            else:
