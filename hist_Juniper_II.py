...
class QuotessSpider(scrapy.Spider):
    name = 'Juniper_II_1661400ARV002'
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/Juniper_II_1661400ARV002/',
        }
    #custom_settings = {
    ...
    #}
    start_urls = ['https://newsroom.juniper.net']
    ...
    def parse(self, response):  # follow drop down menue for different years
         ...
         for year in years:
             aux_url = 'https://newsroom.juniper.net/?bfc50a07_year%5Bvalue%5D=_none&op=Filter&bfc50a07_widget_id=bfc50a07&form_build_id=form-sWS5NFbB5Td4X-tHN_6hkNjh7UC0zw3rY_q7TfVo5BI&form_id=widget_form_base&page={}'
             year_url = [aux_url.format(year)][0]
             yield scrapy.Request(url=year_url, callback=self.parse_next)

    def parse_next(self, response):
          auxs = response.xpath('//article[contains(@class, "nir-widget-list")]')
          for aux in auxs:
              ...
              #        }
              base_url = 'https://newsroom.juniper.net'
    ...
    def parse_details(self, response):
        ...
        name_regex_2=r'(\bAbout\s*Juniper)(.|\s)*|(\bAbout.Juniper\b)(.|\s)*|(\bABOUT.JUNIPER\b)(.|\s)*|(\bABOUT\s*JUNIPER\b)(.|\s)*'
