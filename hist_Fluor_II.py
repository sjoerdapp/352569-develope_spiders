...
class QuotessSpider(scrapy.Spider):
    name = 'Fluor_II_4108000ARV002'
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/Fluor_II_4108000ARV002/',
        }
    #custom_settings = {
    ...
    #}
    start_urls = ['https://newsroom.fluor.com']
    ...
    def parse(self, response):  # follow drop down menue for different years
         ...
         for year in years:
             aux_url = 'https://newsroom.fluor.com/blog/projects?page=0%2C{}'
             year_url = [aux_url.format(year)][0]
             yield scrapy.Request(url=year_url, callback=self.parse_next)

    def parse_next(self, response):
          auxs = response.xpath('//div[@class="view-content"]//ul/li')
          for aux in auxs:
              ...
              item['HEADLINE']= ''.join(aux.xpath('.//div[contains(@class, "field-title")]/span/a//text()').extract())
              ...
              #        }
              base_url = 'https://newsroom.fluor.com'
    ...
    def parse_details(self, response):
        ...
        if '.pdf' in response.url.lower() or 'external.file' in response.url.lower():
            ...
            item['DESCRIPTION'] = re.sub(name_regex,'' ," ".join(response.xpath('//div[@class="panel-pane pane-node-body"]/div[@class="pane-content"]//text()[not(ancestor::div[@class="box__right"] or self::style or self::script or  ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE)
            item['DOCLINK'] = response.url
