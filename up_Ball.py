...
### UPDATES
### scrape actual year and if more news than 20, most recent 20 news
...
class BHGE(scrapy.Spider):
    ...
    start_urls = ['https://www.ball.com/na/newsroom/news-releases'] 
    ...
    def parse(self, response):
        auxs = response.xpath('//section[contains(@class, "listing")]//ul[not(ancestor::div[@class="filter"])]/li')
        if len(auxs)>20:
            auxs = auxs[0:20]
            
        for aux in auxs:
            item = SwisscomIvCrawlerItem()
            item['PUBSTRING'] = aux.xpath('.//h4/text()').extract_first()
            ...
            #        }
            base_url = 'https://www.ball.com/na/newsroom/'
    ...
    def parse_details(self, response):
        ...
        if 'external.file' in response.url.lower():
            ...
            item['DESCRIPTION'] = re.sub(name_regex,'' ," ".join(response.xpath(' //section[@class="mod mod-nws-rls detail"]//text()[not(ancestor::h2)][not(ancestor::div[@class="box__right"] or self::style or self::script or  ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE)
            item['DOCLINK'] = response.url
