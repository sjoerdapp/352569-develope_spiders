...
class QuotessSpider(scrapy.Spider):
    name = 'McCorm_II_2801000ARV002'
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/McCorm_II_2801000ARV002/',
        }
    #custom_settings = {
    ...
    def parse(self, response):  # follow drop down menue for different years
         years = list(range(0, 20, 3)) # fill in years which should be scraped, always last yeat +1 as upper bound will not be element of the list
    ...
    def parse_details(self, response):
        ...
        name_regex = r'(Forward(.|\s*)Looking\s*Statements)(.|\s)*|(Forward(.|\s*)Looking\s*Information)(.|\s)*|(\bABOUT\s*McCormick\b)(.|\s)*|(\bABOUT.McCormick\b)(.|\s)*'
