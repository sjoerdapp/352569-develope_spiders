...
class QuotesInfiniteScrollSpider(scrapy.Spider):
    ...
    def parse(self, response):  # follow drop down menue for different years
         years = list(range(1, 90)) # fill in years which should be scraped, always last yeat +1 as upper bound will not be element of the list
    ...
    def parse_details(self, response):
        ...
        if '.pdf' in response.url.lower():
            ...
            item['DESCRIPTION'] = re.sub(r'(\bAbout\s*Entergy\b)(.|\s)*|(\bAbout.Entergy\b)(.|\s)*|(Entergy\s*Corporation\s*(\(NYSE: ETR\))? is\s*an\s*integrated\s*energy\s*company\s*engaged)(.|\s)*','' ," ".join(response.xpath('//article[@class="large-frame lift"]/p[not(self::style or self::script or descendant::style or descendant::script)][not(@class="lead" or descendant::a[@href= "/news/"])]//text()').extract()), flags=re.IGNORECASE)
            item['DOCLINK'] = response.url
