...
### UPDATES
### latest 20 news, 50 per page

### Fortune Brands Home & Security Inc. 1|1
...
class QuotessSpider(scrapy.Spider):
    name = 'FortuneBrands_5387600ARV001'
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/FortuneBrands_5387600ARV001/',
        }
    #custom_settings = {
    ...
    def parse(self, response):  # follow drop down menue for different years
         ...
         for year in years:
             aux_url = 'https://ir.fbhs.com/press-releases?bbb90a01_items_per_page=50&bbb90a01_year%5Bvalue%5D=_none&op=Filter&bbb90a01_widget_id=bbb90a01&form_build_id=form-wGHyoUx4InkDSjxZgPJ8qFj3Wnlzoa0dtnugrvMxot8&form_id=widget_form_base&page={}'
             year_url = [aux_url.format(year)][0]
             yield scrapy.Request(url=year_url, callback=self.parse_next)

    def parse_next(self, response):
          auxs = response.xpath('//article[@class="row-press-content"]')
          for aux in auxs[0:20]:
              ...
              item['DOCLINK']= aux.xpath('.//a/@href[not(ancestor::ul)]').extract_first()
              #item = {
              #        'PUBSTRING': aux.xpath('./p[@class="news-card-date"]//text()').extract()[1],
              #        'HEADLINE': aux.xpath('.//h3[@class="news-card-title"]/a//text()').extract_first(),
              #        'DOCLINK': aux.xpath('.//h3[@class="news-card-title"]/a/@href').extract_first(),
              #        }
              base_url = 'https://ir.fbhs.com'
    ...
    def parse_details(self, response):
        ...
        name_regex_2=r'(\bAbout\s*Fortune\b)(.|\s)*|(\bAbout.Fortune\b)(.|\s)*|(\bABOUT.FORTUNE\b)(.|\s)*'
