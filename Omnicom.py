...
### 1 spider IR All Reseases; special structure
### first get headline request
### than get request for two different news parts Omnicom and Agency
### they differ by CategoryId
### important to scrape all three sources as latest news are assigned (randomly)to different sources
### descriptions come with json request with the Getpressrelease source
### need to use workflow id to access content

class QuotesInfiniteScrollSpider(scrapy.Spider):
    name = "Omnicom_1367000ARV001"
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/Omnicom_1367000ARV001/',
        }
    ...
    def parse(self, response):  # follow drop down menue for different years
        ...
        cat_ids = ["1cb807d2-208f-4bc3-9133-6a9ad45ac3b0",
                 "ad93ec78-aeb6-42ec-8e84-5e43a4716272"] # fill in years which should be scraped, always last yeat +1 as upper bound will not be element of the list
    ...
    def parse_next(self, response):
        body = response.body
        dats = json.loads(re.split('\d\(',body.decode("utf-8"))[1].split(');')[0])
        ...
        if len(dats['GetPressReleaseListResult']) == 4:
            next_url = response.url.split('pageNumber')[0]
            next_page = str(int(response.url.split('pageNumber=')[1].split('&page')[0]) +1)
            next_page_url = next_url + next_page + '&pageSize=4&pressReleaseDateFilter=1'
    ...
    def parse_details(self, response):
        ...
        if '.pdf' in response.url.lower():
            ...
            item['DESCRIPTION'] = re.sub(r'(\bAbout\s*Omnicom\s*Group\b)(.|\s)* | (\bAbout.Omnicom.Group\b)(.|\s)*','' ," ".join(response.xpath('//div[contains(@class, "article-content")]/*[not(self::style or self::script or descendant::style or descendant::script)]//text()').extract()), re.IGNORECASE)
            item['DOCLINK'] = response.url
            if not item['DESCRIPTION']:
                item['DESCRIPTION'] = re.sub(r'(\bAbout\s*Omnicom\s*Group\b)(.|\s)* | (\bAbout.Omnicom.Group\b)(.|\s)*','' ," ".join(response.xpath('//div[@class="q4default"]/*[not(self::style or self::script or descendant::style or descendant::script)]//text()').extract()))
                yield item
            else:
                yield item
    ...
    def parse_details_II(self, response):
        ...
        body = response.body
        dats = json.loads(re.split('\d\(',body.decode("utf-8"))[1].split(');')[0])
        item['DESCRIPTION'] = re.sub(r'(\bAbout\s*Omnicom\s*Group\b)(.|\s)* | (\bAbout.Omnicom.Group\b)(.|\s)*','' ," ".join(Selector(text=dats['GetPressReleaseItemResult']['Body']).xpath('//div[@class="xn-content"]/*[not(self::style or self::script or descendant::style or descendant::script)]//text()').extract()))
        item['DOCLINK'] = response.url
        yield item
