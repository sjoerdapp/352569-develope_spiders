...
class QuotessSpider(scrapy.Spider):
    ...
    #}
    start_urls = ['http://phx.corporate-ir.net/phoenix.zhtml?c=130560&p=irol-newsarchive']
    ...
    def parse_details(self, response):
        ...
        if '.pdf' in response.url.lower() or 'external.file' in response.url.lower():
            ...
            item['DESCRIPTION'] = re.sub(name_regex,'' ," ".join(response.xpath('//span[@class="ccbnTxtNews"]//text()[not(ancestor::div[@class="box__right"] or self::style or self::script or  ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE)
            item['DOCLINK'] = response.url
