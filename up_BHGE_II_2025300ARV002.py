...
### UPDATES
### first 3 pages plus headline, 24 latest news
...
class BHGE(scrapy.Spider):
    ...
    start_urls = ['https://www.bakerhughes.com/company/newsroom']
    #count = 0
    
    def parse(self, response):
    #def start_requests(self):
        auxs = response.xpath('//article[contains(@class, "list-item")]')
        for aux in auxs:
            item = SwisscomIvCrawlerItem()
            item['PUBSTRING'] = aux.xpath('.//p[contains(@class, "pubdate")]/text()').extract_first()
            if not item['PUBSTRING']:
                item['PUBSTRING'] = aux.xpath('.//p[contains(@class, "info--light")]/text()').extract_first()
        ...
        data = {
            ...
            'view_args': 'dyamic_cards_with_filters/109541/0/1666331',
            #'view_path': '/views/ajax',
            'view_path': '/newsroom',
            'view_base_path': '',
            'view_dom_id': '5ff8938440ef9a8bc8e7a2f3fd3535e319a6dee0855f62f05382deaa69c0d31c',
            'pager_element': '0',
            'type': 'news_item',
        ...
        for num in range(0,0):  # loop iterating over different pages of ajax request
            data['page'] = str(num)
    ...
    def parse_details(self, response):
        ...
        if not item['DESCRIPTION']:
            item['DESCRIPTION'] =  re.sub(name_regex,'' ," ".join(response.xpath('//div[@class="article-body__content"]//text()[not(ancestor::div[@class="box__right"] or self::style or self::script or  ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE)
