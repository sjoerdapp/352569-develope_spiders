...
### Cooper Companies Inc. 1|1
### 1 spider IR Press Releases
### normal get request 
### goes back to 19960214


class QuotesInfiniteScrollSpider(scrapy.Spider):
    name = "CooperCompanies_5780000ARV001"
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/CooperCompanies_5780000ARV001/',
        }
    ...
    def start_requests(self):  # follow drop down menue for different years
         ...
         for year in years:
             aux_url = 'http://investor.coopercos.com/press-releases?field_nir_news_date_value%5Bmin%5D=&items_per_page=50&page={}'
    ...
    def parse_next(self, response):
        ...
        for aux in auxs[0:20]:
            ...
            item['HEADLINE']= aux.xpath('.//div[contains(@class, "news-title")]/div/a/text()').extract_first()
            if not item['HEADLINE']:
                item['HEADLINE']= aux.xpath('.//div[contains(@class, "news-title")]/div/text()').extract_first()
            
            item['DOCLINK']= aux.xpath('.//div[contains(@class, "news-title")]/div/a[2]/@href').extract_first()
            if not item['DOCLINK']:
                item['DOCLINK']= aux.xpath('.//span[contains(@class, "pdf")]/a/@href').extract_first()
            ...
            #        }
            base_url = 'http://investor.coopercos.com'
...
#
    ...
    def parse_details(self, response):
        ...
        name_regex_2=r'(\bAbout\s*The\s*Cooper)(.|\s)*|(\bAbout.The.Cooper\b)(.|\s)*|(\bABOUT.The.Cooper\b)(.|\s)*|(\bABOUT\s*.THE.COOPER\b)(.|\s)*|(\bAbout.CooperCompanies\b)(.|\s)*|'
