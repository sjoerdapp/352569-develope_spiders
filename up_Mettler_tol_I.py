...
### UPDATES
### get current year and take latest news, if more than 20 cut off

### Mettler Toledo International 1|2
### 1 spider IR 2nd -5th spider Press Rleases have to scrape all tabs to miss nothing
### normal get request but content comes in DOC
### original source nur javascript 

class QuotesInfiniteScrollSpider(scrapy.Spider):
    name = "Mettler_I_3215800ARV001"
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/Mettler_I_3215800ARV001/',
        }
    ...
    start_urls = ['https://mt.gcs-web.com/news-releases'] 
    ...
    def parse(self, response):
        ...
        for aux in auxs:
            ...
            item['HEADLINE']= aux.xpath('./td/div[contains(@class, "news-title")]//a[not(contains(@class, "summary"))]/text()').extract_first()
            item['DOCLINK']= aux.xpath('./td/div[contains(@class, "news-title")]//a[not(contains(@class, "summary"))]/@href').extract_first()
            #item = {
    ...
    def parse_details(self, response):
        ...
        if '.pdf' in response.url.lower():
            ...
            #    item['DESCRIPTION'] = re.sub(r'(\bAbout\s*Corning\b)(.|\s)* | (\bAbout.Corning.Incorporated\b)(.|\s)*','' ," ".join(response.xpath('//div[@class="module_body"]/div[@class="node__content"]/*[not(self::div[@class="box__right"] or self::style or self::script or descendant::style or descendant::script)]//text()').extract()), flags=re.IGNORECASE)
            #   
            yield item
