...
### normal get request 
### goes back to 20140106


class QuotesInfiniteScrollSpider(scrapy.Spider):
    name = "KeysightTechnologies_II_5503300ARV002"
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/KeysightTechnologies_II_5503300ARV002/',
        }
    ...
    start_urls = ['https://about.keysight.com/en/newsroom/pr/?cat=all&start=1&page=1'] 
    ...
    def parse(self, response):
        auxs = response.xpath('//div[@class="row"]')
        for aux in auxs:
            item = SwisscomIvCrawlerItem()
            item['PUBSTRING'] = aux.xpath('.//p/text()').extract_first()
            item['HEADLINE']= " ".join(aux.xpath('.//a/text()').extract())
            ...
            #        }
            base_url = 'https://about.keysight.com'
...
#
    ...
    def parse_details(self, response):
        ...
        name_regex_2=r'(\bAbout\s*Keysight)(.|\s)*|(\bAbout.Keysight\b)(.|\s)*|(\bABOUT.Keysight\b)(.|\s)*|(\bABOUT\s*.KEYSIGHTS\b)(.|\s)*'
        if '.pdf' in response.url.lower():
            ...
            item['DESCRIPTION'] = re.sub(name_regex,'' ," ".join(response.xpath('//div[@class="abtBlock"]//text()[not(ancestor::div[contains(@style, "float:right")])][not(ancestor::div[@class="box__right"] or self::style or self::script or  ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE)
