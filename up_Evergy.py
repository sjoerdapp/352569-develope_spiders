...
class QuotesInfiniteScrollSpider(scrapy.Spider):
    name = "Evergy_2103500ARV001"
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/Evergy_2103500ARV001/',
        }
    ...
    start_urls = ['http://www.evergyinc.com/news-releases?items_per_page=50#views-exposed-form-widget-news-widget-news-ul'] 
    ...
    def parse(self, response):
        auxs = response.xpath('//div[@class="item-list"]//li')
        for aux in auxs[0:10]:
            item = SwisscomIvCrawlerItem()
            item['PUBSTRING'] = aux.xpath('.//div[contains(@class, "datetimezone")]/div/text()').extract_first()
            item['HEADLINE']= aux.xpath('.//div[contains(@class, "news-title")]/div/text()').extract_first()
            item['DOCLINK']= aux.xpath('.//div[contains(@class, "news-teaser")]/div/a/@href').extract_first()
            #item = {
            ...
            #        }
            base_url = 'http://www.evergyinc.com'
    ...
    def parse_details(self, response):
        ...
        name_regex = r'(Forward(.|\s*)Looking\s*Statements)(.|\s)*|(\bAbout\s*Evergy\b)(.|\s)*|(\bAbout.Evergy\b)(.|\s)*'
