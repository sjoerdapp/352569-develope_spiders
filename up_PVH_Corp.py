...
### normal get with, all info in one page, however content is double json encoded 
### back to 20091028


class QuotessSpider(scrapy.Spider):
    name = 'PVH_Corp_2140200ARV001'
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/PVH_Corp_2140200ARV001/',
        }
    #custom_settings = {
    ...
    #}
    start_urls = ['https://www.pvh.com/sitecore/api/pvh/nasdaq/cache?reqtype=news&_=1563522656489']
    ...
    def parse(self, response):
          body = json.loads(json.loads(response.text))
          for aux in body['data'][0:20]:
              item = SwisscomIvCrawlerItem()
              item['PUBSTRING'] = aux['releaseDate']['date'].split('T')[0] # cuts out the part berfore the date as well as the /n at the end of the string
              item['HEADLINE']= aux['title']
              item['DOCLINK']= aux['body'][0]['link']['url']
              #item = {
    ...
    def parse_details(self, response):
        ...
        name_regex_2=r'(\bAbout\s*PVH)(.|\s)*|(\bAbout.PVH\b)(.|\s)*|(\bABOUT.PVH\b)(.|\s)*|(\bABOUT\s*PVH\b)(.|\s)*'
        ...
        if '.pdf' in response.url.lower() or 'external.file' in response.url.lower():
            ...
            item['DESCRIPTION'] = re.sub(name_regex,'' ," ".join(Selector(text=body).xpath('//text()[not(ancestor::div[@class="box__right"] or self::style or self::script or ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE)
