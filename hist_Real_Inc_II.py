...
### Realty Income Corp 2|2 2013 - 2005
### scrapes history starting 2013 für 2013 2HJ press releases contain all information, however use pdfs for complete 2013
### at all 2 spiders: I inkl 2014, II 2013 - 2005
### historic press releases to short -> need to get pdf. starting 2013
### at all back to 20050727

class QuotesInfiniteScrollSpider(scrapy.Spider):
    name = "Real_Inc_II_3050000ARV002"
    
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/Real_Inc_II_3050000ARV002/',
        }
    ...
    start_urls = ['https://www.realtyincome.com/investors/press-releases/2019/default.aspx'] 

    
    def parse(self, response):  # follow drop down menue for different years
         years = list(range(2005, 2014)) # fill in years which should be scraped, always last yeat +1 as upper bound will not be element of the list
         #del years[0]  # delets first element "NULL" from list of years
         for year in years:
             aux_url = 'https://www.realtyincome.com/investors/press-releases/{}/default.aspx'
    ...
    def parse_next(self, response):
        auxs = response.xpath('//div[@class="ModuleContentContainer"]/div[contains(@class, "ModuleItemRow")]')
        for aux in auxs:
            item = SwisscomIvCrawlerItem()
            item['PUBSTRING'] = aux.xpath('.//span[@class= "ModuleDate"]/text()').extract_first()
            item['HEADLINE']= aux.xpath('.//span[@class= "ModuleHeadline"]/text()').extract_first().split("\n    ")[1].split("\n")[0]
            #item['DOCLINK']= aux.xpath('.//a/ @href').extract_first()
            #item = {
            #        'PUBSTRING': aux.xpath('.//span[@class= "ModuleDate"]/text()').extract_first(),
            #        'HEADLINE': aux.xpath('.//span[@class= "ModuleHeadline"]/text()').extract_first().split("\n    ")[1].split("\n")[0],
            #        'DOCLINK': aux.xpath('.//a/ @href').extract_first(),
            #        }
            base_url = 'https:'
            url= base_url + aux.xpath('.//a[contains(@href, ".pdf")]/@href ').extract_first()
            item['file_urls'] = [url]
            item['DOCLINK']= url
            item['DESCRIPTION'] = ''
    ...
    #    item['DESCRIPTION'] = re.sub(r'(\bAbout\s*the\s*Company\b)(.|\s)* | (\bAbout.the.Company\b)(.|\s)*','' ," ".join(response.xpath('//div[@class="ModuleBody"]//text()').extract()))
