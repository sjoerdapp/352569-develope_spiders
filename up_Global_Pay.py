...
### UPDATES
### take first two pages, 10 items per page

### Global Payments Inc 1||1
### 1 spider IR All Reseases
### normal get request and potentially all pdfs
### back to 20090106

class QuotesInfiniteScrollSpider(scrapy.Spider):
    name = "Gl_Pay_4116800ARV001"
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/Gl_Pay_4116800ARV001/',
        }
    ...
    start_urls = ['https://investors.globalpaymentsinc.com/news-and-events/press-releases?items_per_page=10&page=1'] 
    ...
    def parse(self, response):  # follow drop down menue for different years
         ...
         for year in years:
             aux_url = 'https://investors.globalpaymentsinc.com/news-and-events/press-releases?items_per_page=10&page={}'
    ...
    def parse_next(self, response):
        ...
        for aux in auxs:
            ...
            item['PUBSTRING'] = aux.xpath('.//div[@class="field-content"]//div[@class="field__item"]/text()').extract_first()
            ...
            #        }
            base_url = 'https://investors.globalpaymentsinc.com'
            aux_url = aux.xpath('.//div[@class="field-content"]/a[2]/@href').extract_first()
    ...
    def parse_details(self, response):
        ...
        name_regex = r'(Global\s*Payments\s*Inc.\s*(\(NYSE:\s*GPN\))?\s*is\s*a\s*leading\s*provider)(.|\s)*|(Forward(.|\s*)Looking\s*Statements)(.|\s)*|(\bAbout\s*Global\s*Payments\b)(.|\s)*|(\bAbout.Gobal.Payments\b)(.|\s)*'
        item['DESCRIPTION'] = re.sub(r"\(\s*Logo:\s*ht(.*)LOGO\s*\)|\(\s*Photo:\s*ht(.*)LOGO\s*\)", '', re.sub(name_regex,'' ," ".join(response.xpath('//div[@class="node__content"]//text()[not(ancestor::div[@class="box__right"] or self::style or self::script or  ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE))
