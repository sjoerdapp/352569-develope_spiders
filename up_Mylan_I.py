...
### normal get request goes back until 20040429
### goes back

class QuotesInfiniteScrollSpider(scrapy.Spider):
    name = "Mylan_3104000ARV001"
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/Mylan_3104000ARV001/',
        }
    ...
    start_urls = ['http://newsroom.mylan.com/press-releases'] 

    
    def parse(self, response):  # follow drop down menue for different years
         years = list(range(0,11, 10)) # fill in years which should be scraped, always last yeat +1 as upper bound will not be element of the list
         #del years[0]  # delets first element "NULL" from list of years
         for year in years:
             aux_url = 'http://newsroom.mylan.com/press-releases?o={}'
    ...
    def parse_next(self, response):
        auxs = response.xpath('//ul[@class="wd_layout-simple wd_item_list"]/li')
        for aux in auxs:
            ...
            #        }
            base_url = 'http://newsroom.mylan.com'
    ...
    def parse_details(self, response):
        ...
        if '.pdf' in response.url.lower():
            ...
            item['DESCRIPTION'] = re.sub(r'(Forward(.|\s*)Looking\s*Statements)(.|\s)*|(Mylan\s*is\s*a\s*global\s*pharmaceutical\s*company\s*committed\s*to\s*setting)(.|\s)*|(\bAbout\s*Mylan\b)(.|\s)* | (\bAbout.Mylan\b)(.|\s)*','' ," ".join(response.xpath('//div[@class="wd_body wd_news_body"]/*[not(self::style or self::script or descendant::style or descendant::script)]//text()[not(ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE)
            item['DOCLINK'] = response.url
