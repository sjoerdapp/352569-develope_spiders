...
### 1 spider IR News Releases long history back to 2005 -  classic Newsroom much shorter Historie wird nicht gescraped
### normal get request with xpath
### history back to 2005, 387 News'

class QuotesInfiniteScrollSpider(scrapy.Spider):
    name = "Ameripri_4742200ARV001"
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/Ameripri_4742200ARV001/',
        }
    ...
    start_urls = ['http://ir.ameriprise.com/News/113901/NewsData?pageIndex=1'] 

    
    def parse(self, response):  # follow drop down menue for different years
         years = list(range(0, 79)) # fill in years which should be scraped, always last yeat +1 as upper bound will not be element of the list
         #del years[0]  # delets first element "NULL" from list of years
         for year in years:
             aux_url = 'http://ir.ameriprise.com/News/113901/NewsData?pageIndex={}'
    ...
    def parse_next(self, response):
        ...
        for aux in auxs:
            ...
            #        }
            base_url = 'http://ir.ameriprise.com'
    ...
    def parse_details(self, response):
        ...
        if '.pdf' in response.url.lower():
            ...
            name_regex = r'(Forward(.|\s*)Looking\s*Statement)(.|\s)*|(Ameriprise\s*Financial.\s*Inc.\s*is\s*a\s*leading)(.|\s)*|(Forward(.|\s*)Looking\s*Statements)(.|\s)*|(\bAbout\s*Ameriprise\s*Financial\b)(.|\s)* | (\bAbout.Ameriprise.Financial\b)(.|\s)*'
            item['DESCRIPTION'] = re.sub(name_regex,'' ," ".join(response.xpath('//div[@class="irwFilePageBody"]/*[not(self::style or self::script or descendant::style or descendant::script)]//text()[not(ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE)
            item['DOCLINK'] = response.url
