...
###  spider IR Publications
### normal get request
### goes back to 20140415

class QuotesInfiniteScrollSpider(scrapy.Spider):
    name = "Entergy_II_2120100ARV002"
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/Entergy_II_2120100ARV002/',
        }
    ...
    start_urls = ['https://entergycorporation.gcs-web.com/publications'] 
    ...
    def parse(self, response):  # follow drop down menue for different years
         ...
         for year in years:
             aux_url = 'https://entergycorporation.gcs-web.com/publications?aeb90975_year%5Bvalue%5D=_none&aeb90975_widget_id=aeb90975&form_build_id=form-Fcl1sxUmECNyv7lbSwSo_kb5g243Wou84qBRxnEvTm0&form_id=widget_form_base&page={}'
    ...
    def parse_next(self, response):
        auxs = response.xpath('//div[@class="nir-widget--list"]/div[@class="wrapper"]')
        for aux in auxs:
            ...
            #        }
            base_url = 'https://entergycorporation.gcs-web.com'
    ...
    def parse_details(self, response):
        ...
        if '.pdf' in response.url.lower():
            ...
            item['DESCRIPTION'] = re.sub(r'(\bAbout\s*Entergy\b)(.|\s)* | (\bAbout.Entergy\b)(.|\s)*','' ,"".join(response.xpath('//article[@class="large-frame lift"]/p[not(self::style or self::script or descendant::style or descendant::script)][not(@class="lead" or descendant::a[@href= "/news/"])]//text()').extract()), flags=re.IGNORECASE)
            item['DOCLINK'] = response.url
