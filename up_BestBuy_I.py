...
class QuotessSpider(scrapy.Spider):
    name = 'BestBuy_I_7506000ARV001'
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/BestBuy_I_7506000ARV001/',
        }
    #custom_settings = {
    ...
    #}
    start_urls = ['http://investors.bestbuy.com/investor-relations/news-and-events/financial-releases']

    def parse(self, response):  # follow drop down menue for different years
         years = list(range(2019, 2021))
         #del years[0]  # delets first element "NULL" from list of years
         for year in years:
             aux_url = 'http://investors.bestbuy.com/investor-relations/news-and-events/financial-releases/{}/default.aspx'
             year_url = [aux_url.format(year)][0]
             yield scrapy.Request(url=year_url, callback=self.parse_next)

    def parse_next(self, response):
          auxs = response.xpath('//div[contains(@class,"ModulePressRelease")]//div[@class="ModuleContainerInner"]/div[contains(@class, "ModuleItemRow")]')
          if len(auxs) > 20:
            auxs = auxs[0:20]
          ...
          for aux in auxs:
              ...
              item['HEADLINE']= aux.xpath('./a[@class="ModuleHeadlineLink"]//text()').extract_first()
              ...
              #        }
              base_url = 'http://investors.bestbuy.com'
    ...
    def parse_details(self, response):
        ...
        name_regex = r'(Forward(.|\s*)Looking\s*Statements)(.|\s)*|(\bABOUT\s*Best\s*Buy\b)(.|\s)*|(\bABOUT.Best.Buy\b)(.|\s)*' #|(\bABOUT\s*L\s*BRANDS\b)(.|\s)*'
