...
### BorgWarner Inc 2|2
### 1st spider Press Releases, 2nd spider Awards
### normal get, all news in one page
### 20050419

class BHGE(scrapy.Spider):
    name = "BorgWarner_II_2993600ARV002"
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/BorgWarner_II_2993600ARV002/',
        }
    #custom_settings = {
    ...
    #}
    start_urls = ['https://www.borgwarner.com/newsroom/awards']
    ...
    def parse_next(self, response):
          auxs = response.xpath('//div[contains(@class, "column widget-block large")]')
          for aux in auxs:
    ...
    def parse_details(self, response):
        ...
        if '.pdf' in response.url.lower() or 'external.file' in response.url.lower():
            ...
            item['DESCRIPTION'] = re.sub(name_regex,'' ," ".join(response.xpath('//span[@id="MainContent_C009_spanBodyText"]//text()[not(ancestor::div[@class="box__right"] or self::style or self::script or  ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE)
            item['DOCLINK'] = response.url
