...
### 608 News back to 19991007

class QuotesInfiniteScrollSpider(scrapy.Spider):
    name = "aligntech_4115400ARV001"
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/aligntech_4115400ARV001/',
        }
    ...
    start_urls = ['http://investor.aligntech.com/company-information/news-release'] 

    
    def parse(self, response):  # follow drop down menue for different years
         years = list(range(0, 31)) # fill in years which should be scraped, always last yeat +1 as upper bound will not be element of the list
         #del years[0]  # delets first element "NULL" from list of years
         for year in years:
             aux_url = 'http://investor.aligntech.com/company-information/news-release?page={}'
    ...
    def parse_next(self, response):
        ...
        for aux in auxs:
            ...
            item['PUBSTRING'] = aux.xpath('./td/div[@class="datetime"]/text()').extract_first()
            ...
            #        }
            base_url = 'http://investor.aligntech.com'
    ...
    def parse_details(self, response):
        ...
        name_regex = r'(Forward(.|\s*)Looking\s*Statement)(.|\s)*|(Forward(.|\s*)Looking\s*Statements)(.|\s)*|(\bAbout\s*Align\s*Technology\b)(.|\s)* | (\bAbout.Align.Technology\b)(.|\s)*'
