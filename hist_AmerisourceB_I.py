...
class QuotessSpider(scrapy.Spider):
    name = 'AmeriB_I_1365000ARV001'
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/AmeriB_I_1365000ARV001/',
        }
    #custom_settings = {
    ...
    #}
    start_urls = ['http://investor.amerisourcebergen.com/news-releases']
    ...
    def parse(self, response):
          auxs = response.xpath('//div[@class="item-list"]//div[@class="col-lg-4"]')
          for aux in auxs:
              ...
              item['HEADLINE']= aux.xpath('.//div[@class="card__title"]/a/text()').extract_first()
              item['DOCLINK']= aux.xpath('.//div[@class="card__title"]/a/@href').extract_first()
              #item = {
              #        'PUBSTRING': aux.xpath('./p[@class="news-card-date"]//text()').extract()[1],
              #        'HEADLINE': aux.xpath('.//h3[@class="news-card-title"]/a//text()').extract_first(),
              #        'DOCLINK': aux.xpath('.//h3[@class="news-card-title"]/a/@href').extract_first(),
              #        }
              base_url = 'http://investor.amerisourcebergen.com'
              aux_url = aux.xpath('.//div[@class="card__title"]/a/@href').extract_first()
    ...
    def parse_details(self, response):
        ...
        name_regex = r'(Forward(.|\s*)Looking\s*Statements)(.|\s)*|(\bABOUT\s*Amerisource)(.|\s)*|(\bABOUT.Amerisource)(.|\s)*' #|(\bABOUT\s*L\s*BRANDS\b)(.|\s)*'
