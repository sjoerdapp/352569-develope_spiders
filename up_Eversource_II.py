...
### 2nd spider massachusetts
### have to converst timestamp
### classic post with payload and json content

class BHGE(scrapy.Spider):
    name = "EverS_II_2129900ARV002"
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/EverS_II_2129900ARV002/',
        }
    ...
    def start_requests(self):
        headers = {
            ...
            'Referer': 'https://www.eversource.com/content/ct-c/about/news-room/massachusetts/massachusetts-news',
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/71.0.3578.98 Safari/537.36',
            #'X-NewRelic-ID': 'VQYBUlRVChABXFNXBAcCXw==',
            'X-Requested-With': 'XMLHttpRequest',
           }

        data = {"request":{"NewsgroupId":"8a46880f-1b52-67e3-9dbd-ff0000e2e88e","DetailsPageId":"6bbe890f-1b52-67e3-9dbd-ff0000e2e88e","ListPageUrl":"www.eversource.com/content/ct-c/about/news-room/massachusetts/massachusetts-news","CurrentPage":5,"ItemsPerPage":5,"DateFilter":None,"CategoryFilter":None,"TagFilter":None}}
    ...
    def parse(self, response): 
        ...
        data = {"request":{"NewsgroupId":"8a46880f-1b52-67e3-9dbd-ff0000e2e88e","DetailsPageId":"6bbe890f-1b52-67e3-9dbd-ff0000e2e88e","ListPageUrl":"www.eversource.com/content/ct-c/about/news-room/massachusetts/massachusetts-news","CurrentPage":1,"ItemsPerPage":5,"DateFilter":None,"CategoryFilter":None,"TagFilter":None}}
    ...
    def parse_details(self, response):
        ...
        name_regex = r'(Eversource\s*\(\s*NYSE\s*:\s*ES\s*\)\s*transmits\s*and\s*delivers\s*electricity(.\s*|\s*and\s*)natural\s*gas)(.|\s)*|(Eversource\s*\(\s*NYSE\s*:\s*ES\s*\)\s*transmits\s*and\s*delivers\s*electricity\s*to)(.|\s)*|(Forward(.|\s*)Looking\s*Statements)(.|\s)*|(\bAbout\s*Eversource:\b)(.|\s)*| (\bAbout.Eversource:\b)(.|\s)*'
        ...
        if not item['DESCRIPTION']:
            item['DESCRIPTION'] = re.sub(name_regex,'' ," ".join(response.xpath('//div[@class="article-content"]//div[@style="text-align: left;"]//text()[not(ancestor::div[@class="box__right"] or self::style or self::script or  ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE)
            if not item['DESCRIPTION']:
