...
### UPDATE
### take first 2 pages correspondes to 18 (9 News per page)

### Entergy Corporation 1||2
### 1 spider Newsroom 2nd spider IR Publications
### normal get request
### goes back to 20131219

class QuotesInfiniteScrollSpider(scrapy.Spider):
    name = "Entergy_I_2120100ARV001"
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/Entergy_I_2120100ARV001/',
        }
    ...
    #start_urls = ['https://www.entergynewsroom.com/news'] 

    
    def start_requests(self):  # follow drop down menue for different years
         years = list(range(1, 3)) # 137 last histroy; fill in years which should be scraped, always last yeat +1 as upper bound will not be element of the list
         #del years[0]  # delets first element "NULL" from list of years
         for year in years:
             aux_url = 'https://www.entergynewsroom.com/news/?page={}'
    ...
    def parse_next(self, response):
        auxs = response.xpath('//main/div[@class="row"]/div[contains(@class, "medium-8")]//a')
        for aux in auxs:
            item = SwisscomIvCrawlerItem()
            item['PUBSTRING'] = aux.xpath('.//div[@class="date"]/text()').extract_first()
            item['HEADLINE']= aux.xpath('.//div[@class="title"]/text()').extract_first()
            ...
            #        }
            base_url = 'https://www.entergynewsroom.com'
            aux_url = aux.xpath('./@href').extract_first()
    ...
    def parse_details(self, response):
        ...
        if '.pdf' in response.url.lower():
            ...
            item['DESCRIPTION'] = re.sub(r'(\bAbout\s*Entergy\b)(.|\s)*|(\bAbout.Entergy\b)(.|\s)*|(Entergy\s*Corporation\s*(\(NYSE: ETR\))? is\s*an\s*integrated\s*energy\s*company\s*engaged)(.|\s)*','' ," ".join(response.xpath('//article[@class="large-frame lift"]//text()[not(ancestor::p[contains(text(), "> ")])][not(ancestor::h1 or ancestor::a[contains(text(),"News")]  or ancestor::div[@class= "large"] or ancestor::p[@class= "lead"] or ancestor::div[@class= "row collapse contact-box"])][not(ancestor::div[@class="box__right"] or self::style or self::script or  ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE)
            item['DOCLINK'] = response.url
            if not item['DESCRIPTION']:
                item['DESCRIPTION'] = re.sub(r'(\bAbout\s*Entergy\b)(.|\s)*|(\bAbout.Entergy\b)(.|\s)*|(Entergy\s*Corporation\s*(\(NYSE: ETR\))? is\s*an\s*integrated\s*energy\s*company\s*engaged)(.|\s)*','' ," ".join(response.xpath('//article[@class="large-frame lift"]/table//tr[3]/*[not(self::style or self::script or descendant::style or descendant::script)]//text()').extract()), flags=re.IGNORECASE)
                yield item 
            else:   
                yield item
