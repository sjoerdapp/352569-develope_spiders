...
### 1 spider IR Press Releases, 2nd Newsroom minor but significant differences between two sources
### normal get requests
### need to follow pagination links
### back to 20160311

class QuotesInfiniteScrollSpider(scrapy.Spider):
    name = "LambWeston_II_9900143ARV002"
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/LambWeston_II_9900143ARV002/',
        }
    ...
    start_urls = ['https://news.lambweston.com/press-releases/all/all/all'] 

    
    #def parse(self, response):  # follow drop down menue for different years
    #     years = list(range(2016, 2020)) # fill in years which should be scraped, always last yeat +1 as upper bound will not be element of the list
    #     #del years[0]  # delets first element "NULL" from list of years
    #     for year in years:
    #         aux_url = 'https://investors.lambweston.com/news-releases/{}'
    ...
    def parse(self, response):
        auxs = response.xpath('//div[contains(@class, "view-content")]//ul/li')
        for aux in auxs:
            item = SwisscomIvCrawlerItem()
            item['PUBSTRING'] = aux.xpath('.//div[@class="views-field-item views-field-created"]/span/text()').extract_first()
            item['HEADLINE']= aux.xpath('.//div[@class="views-field-item views-field-title"]//a/text()').extract_first()
            item['DOCLINK']= aux.xpath('.//div[@class="views-field-item views-field-title"]//a/@href').extract_first()
            #item = {
            ...
            #        }
            base_url = 'https://news.lambweston.com'
        ...
        #if next_page_url:
        #    next_page_url = 'https://investors.lambweston.com' + next_page_url
    ...
    def parse_details(self, response):
        ...
        name_regex_2=r'(\bAbout\s*Lamb)(.|\s)*|(\bAbout.Lamb\b)(.|\s)*|(\bABOUT.Lamb\b)(.|\s)*|(\bABOUT\s*.LAMB\b)(.|\s)*'
        if '.pdf' in response.url.lower():
            ...
            item['DESCRIPTION'] = re.sub(name_regex,'' ," ".join(response.xpath('//div[@class="panel-pane pane-pr-body"]//text()[not(ancestor::div[@class="box__right"] or self::style or self::script or  ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE)
