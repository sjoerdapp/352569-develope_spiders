...
### normal get request potentially pdfs for almost all news
###  goes back to 19980120

class QuotesInfiniteScrollSpider(scrapy.Spider):
    name = "KLA_Tencor_2551000ARV001"
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/KLA_Tencor_2551000ARV001/',
        }
    ...
    start_urls = ['http://ir.kla-tencor.com/press-releases'] 

    
    def parse(self, response):  # follow drop down menue for different years
         years = list(range(0, 51)) # fill in years which should be scraped, always last yeat +1 as upper bound will not be element of the list
         #del years[0]  # delets first element "NULL" from list of years
         for year in years:
             aux_url = 'http://ir.kla-tencor.com/press-releases?aa450940_year%5Bvalue%5D=_none&op=Filter&aa450940_widget_id=aa450940&form_build_id=form-zrl3RJqo5U5gwBlC8rYduZQJVWEnms1lIhvDv-ZHsrk&form_id=widget_form_base&page={}'
    ...
    def parse_next(self, response):
        ...
        for aux in auxs:
            ...
            #        }
            base_url = 'http://ir.kla-tencor.com'
    ...
    def parse_details(self, response):
        ...
        if '.pdf' in response.url.lower():
            ...
            item['DESCRIPTION'] = re.sub(r'(\bAbout\s*KLA\b)(.|\s)*|(\bAbout.KLA\b)(.|\s)*|(Forward-?\s*Looking Statements:)(.|\s)*','' ," ".join(response.xpath('//div[@class="xn-content"]/*[not(self::style or self::script or descendant::style or descendant::script)]//text()').extract()), flags=re.IGNORECASE)
            item['DOCLINK'] = response.url
            if not item['DESCRIPTION']:
                item['DESCRIPTION'] = re.sub(r'(\bAbout\s*KLA\b)(.|\s)* | (\bAbout.KLA\b)(.|\s)*|(Forward-?\s*Looking Statements:)(.|\s)*','' ," ".join(response.xpath('//div[contains(@class, "main-block")]/article/div[@class="node__content"]//text()[not(ancestor::div[@class="box__right"] or self::style or self::script or ancestor::style or ancestor::script)]').extract()), flags=re.IGNORECASE)
                yield item
            else:    
                yield item
