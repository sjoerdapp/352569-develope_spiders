...
### 2nd spider Press Releases, 3rd spider In the News, 4th spider Blog
### normal get all news in one page
### back to 20130106

### Achtung about sealed air wird automatisch abgeschnitten, checken ob das so passt

class QuotessSpider(scrapy.Spider):
    name = 'SealedAir_II_3470200ARV002'
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/SealedAir_II_3470200ARV002/',
        }
    #custom_settings = {
    ...
    #}
    start_urls = ['https://sealedair.com/press-releases']
    ...
    def parse(self, response):
          auxs = response.xpath('//div[contains(@class,"view view-media-center-views view-id-media_center_views view-display-id-block view-dom-id")]//div[contains(@class, "views-row")]')
          for aux in auxs[0:20]:
              item = SwisscomIvCrawlerItem()
              item['PUBSTRING'] = aux.xpath('.//span[@class="date"]/span/text()').extract_first() # cuts out the part berfore the date as well as the /n at the end of the string
              head_I= aux.xpath('.//span[@class="title"]/text()[1]').extract_first()
              item['HEADLINE'] = head_I
              head_II = aux.xpath('.//span[@class="title"]/text()[2]').extract_first()
              if head_II:
                item['HEADLINE'] = head_I + ' ' + head_II
              item['DOCLINK']= aux.xpath('.//span[@class="field-content"]/a/@href').extract_first()
              #item = {
