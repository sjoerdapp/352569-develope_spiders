...
class QuotessSpider(scrapy.Spider):
    name = 'Marathon_oil_2179500ARV001'
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/Marathon_oil_2179500ARV001/',
        }
    #custom_settings = {
    ...
    #}
    start_urls = ['https://ir.marathonoil.com']
    ...
    def parse(self, response):  # follow drop down menue for different years
         ...
         for year in years:
             aux_url = 'https://ir.marathonoil.com/news-releases?l=50&o={}'
    ...
    def parse_next(self, response):
          ...
          for aux in auxs:
              ...
              #        }
              base_url = 'https://ir.marathonoil.com'
    ...
    def parse_details(self, response):
        ...
        name_regex = r'(Forward(.|\s*)Looking\s*Statements)(.|\s)*|(\bIn\s*analyzing\s*and\s*planning\s*for\s*its\s*business.\s*Marathon\s*Oil\s*supplements\s*its\s*use\s*of\s*GAAP\s*financial\s*measures\b)(.|\s)*|(\bMarathon Oil is an\s*(independent\s*)?(international\s*)?exploration and production company\b)(.|\s)*' #|(\bABOUT\s*L\s*BRANDS\b)(.|\s)*'
