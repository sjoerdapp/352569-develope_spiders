...
### Healthpeak properties 2|2
###  1st spider IR releases
### normal get 
### back to 20130116


class QuotessSpider(scrapy.Spider):
    name = 'HCP_I_2089100ARV001'
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/HCP_I_2089100ARV001/',
        }
    #custom_settings = {
    ...
    #}
    start_urls = ['https://ir.healthpeak.com/press-room?l=25']
    ...
    #     for year in years:
    #         aux_url = 'http://ir.hcpi.com/news-releases?o={}'
    #         year_url = [aux_url.format(year)][0]
    #         yield scrapy.Request(url=year_url, callback=self.parse_next)

    def parse(self, response):
          auxs = response.xpath('//ul[contains(@class, "wd_item_list")]/li[@class="wd_item"]')[0:20]
          for aux in auxs:
              ...
              #        }
              base_url = 'https://ir.healthpeak.com/'
    ...
    def parse_details(self, response):
        ...
        name_regex = r'(Forward(.|\s*)Looking\s*Statements)(.|\s)*|(\bABOUT\s*HCP )(.|\s)*|(\bABOUT.HCP )(.|\s)*'
        name_regex_2=r'(\bAbout\s*Healthpeak\b)(.|\s)*|(\bAbout.Healthpeak\b)(.|\s)*'
