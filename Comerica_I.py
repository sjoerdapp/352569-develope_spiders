...
### normal phoenix get all news in one page
### back to 19971121
...
class QuotessSpider(scrapy.Spider):
    ...
    #}
    start_urls = ['http://investor.comerica.com/phoenix.zhtml?c=114699&p=irol-news']
    ...
    def parse(self, response):
          auxs = response.xpath('//div[@id="investorPrimary"]/table[4]/tr/td/table/tr[not(contains(@class, "Ttl"))]')
          for aux in auxs:
              ...
              item['HEADLINE']= aux.xpath('./td/span/a[@class="ccbnTblLnk"]/text()').extract_first()
              item['DOCLINK']= aux.xpath('./td/span/a[@class="ccbnTblLnk"]/@href').extract_first()
              #item = {
              #        'PUBSTRING': aux.xpath('./p[@class="news-card-date"]//text()').extract()[1],
              #        'HEADLINE': aux.xpath('.//h3[@class="news-card-title"]/a//text()').extract_first(),
              #        'DOCLINK': aux.xpath('.//h3[@class="news-card-title"]/a/@href').extract_first(),
              #        }
              base_url = 'http://investor.comerica.com/'
              aux_url = aux.xpath('./td/span/a[@class="ccbnTblLnk"]/@href').extract_first()
    ...
    def parse_details(self, response):
        ...
        if '.pdf' in response.url.lower() or 'external.file' in response.url.lower():
            ...
            item['DESCRIPTION'] = re.sub(name_regex,'' ," ".join(response.xpath('//tr[@class="ccbnBgTxt"]//span[@class="ccbnTxt"]//text()[not(ancestor::img)][not(ancestor::div[@class="box__right"] or self::style or self::script or  ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE)
            item['DOCLINK'] = response.url
