...
class QuotessSpider(scrapy.Spider):
    name = 'NielsenHoldings_I_5365600ARV001'
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/NielsenHoldings_I_5365600ARV001/',
        }
    #custom_settings = {
    ...
    #}
    start_urls = ['https://ir.nielsen.com']
    ...
    def parse(self, response):  # follow drop down menue for different years
         ...
         for year in years:
             aux_url = 'https://ir.nielsen.com/investor-relations/shareholder-information/press-releases/{}/default.aspx'
             year_url = [aux_url.format(year)][0]
             yield scrapy.Request(url=year_url, callback=self.parse_next)

    def parse_next(self, response):
          auxs = response.xpath('//div[contains(@class, "ModuleItem")]')
          for aux in auxs:
              ...
              item['HEADLINE']= aux.xpath('.//span[@class="ModuleHeadline"]/text()').extract_first().upper()
    ...
    def parse_details(self, response):
        ...
        if '.pdf' in response.url.lower() or 'external.file' in response.url.lower():
            ...
            item['DOCLINK'] = response.url
            pdf_link = response.xpath('//div[@class="ModuleLinks"]/a/@href').extract_first()
            if pdf_link:
