...
### UPDATES
### all news one page get latest 25 news 

### Cintas Corporation 1||1
### 1 spider IR Press Releases
### All Reseases one page
### Pubstring from text in detailpage
### normal get request
### back to 20130202

class QuotesInfiniteScrollSpider(scrapy.Spider):
    name = "Cintas_6304000ARV001"
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/Cintas_6304000ARV001/',
        }
    ...
    start_urls = ['https://www.cintas.com/news/'] 
    ...
    def parse(self, response):
        auxs = response.xpath('//div[@class="newsTitle"]')
        for aux in auxs[0:20]:
            ...
            item['DOCLINK']= aux.xpath('.//a/@id').extract_first()
            #item = {
            ...
            #        }
            base_url = 'https://www.cintas.com/news/press-releases/'
    ...
    def parse_details(self, response):
        ...
        if '.pdf' in response.url.lower():
            ...
            item['PUBSTRING'] = re.split('(([a-zA-Z]+ \d+, \d{4})| ([a-zA-Z.]+ \d+, \d{4}) |(\[([a-zA-Z]+ \d+, \d{4})\]))', ' '.join(response.xpath('//div[@class="newsSectionText"]//text()[not(ancestor::h1)][not(ancestor::div[@class="box__right"] or self::style or self::script or  ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()).replace('\xa0', ' '))[1]
            if '[' in item['PUBSTRING']:
                item['PUBSTRING']= item['PUBSTRING'].split('[')[1].split(']')[0]
            if '(' in item['PUBSTRING']:
                item['PUBSTRING']= item['PUBSTRING'].split('(')[1].split(')')[0]
            item['DESCRIPTION'] = re.sub(r'(Forward(.|\s*)Looking\s*Statements)(.|\s)*|(\bAbout\s*Cintas\b)(.|\s)* | (\bAbout.Cintas\b)(.|\s)*','' ," ".join(response.xpath('//div[@class="newsSectionText"]//text()[not(ancestor::h1)][not(ancestor::div[@class="box__right"] or self::style or self::script or  ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE)
            item['DOCLINK'] = response.url
