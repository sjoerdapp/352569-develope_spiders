...
### financial press releases not in news archive -> need extra spider to download press releases

class QuotesInfiniteScrollSpider(scrapy.Spider):
    name = "North_T_3275000ARV001"
    ...
    start_urls = ['https://www.northerntrust.com'] 

    
    def parse(self, response):  # follow drop down menue for different years
         years = list(range(0, 500, 10)) # fill in years which should be scraped, always last yeat +1 as upper bound will not be element of the list
         #years.insert(0, 0)
         #del years[0]  # delets first element "NULL" from list of years
         for year in years:
             aux_url = 'https://www.northerntrust.com/about-us/news/press-release-archive?&s={}'
    ...
    def parse_next(self, response):
        auxs = response.xpath('//ul[@class="publicationItems"]/li')
        for aux in auxs:
            item = {
                    'PUBSTRING': aux.xpath('./h3/div[@class="bodyText"]/text()').extract_first(),
                    'HEADLINE': aux.xpath('./h3/a/text()').extract_first(),
                    'DOCLINK': aux.xpath('./h3/a/@href').extract_first(),
                    }
            base_url = 'https://www.northerntrust.com'
    ...
    def parse_details(self, response):
        ...
        item['DESCRIPTION'] = re.sub(r'(\bAbout\s*Northern\s*Trust\b)(.|\s)* | (\bAbout.Northern.Trust\b)(.|\s)*','' ," ".join(response.xpath('//div[@class="bodyText pressReleaseDetail"]//text()').extract()))
