...
### normal get with pagination link
### back to 20120111


class QuotessSpider(scrapy.Spider):
    name = 'Lennar_2109500ARV001'
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/Lennar_2109500ARV001/',
        }
    #custom_settings = {
    ...
    #}
    start_urls = ['https://www.lennar.com/about/about']

    def parse(self, response):  # follow drop down menue for different years
         years = list(range(2012, 2020)) # fill in years which should be scraped, always last yeat +1 as upper bound will not be element of the list
         #del years[0]  # delets first element "NULL" from list of years
         for year in years:
             aux_url = 'http://investors.lennar.com/press-releases/{}'
             year_url = [aux_url.format(year)][0]
             yield scrapy.Request(url=year_url, callback=self.parse_next)

    def parse_next(self, response):
          auxs = response.xpath('//table//tr[not(contains(@class, "Headers"))]')
          for aux in auxs:
              item = SwisscomIvCrawlerItem()
              item['PUBSTRING'] = aux.xpath('./th[@class="pr-date-field"]/text()').extract_first() # cuts out the part berfore the date as well as the /n at the end of the string
              ...
              #        }
              base_url = 'http://investors.lennar.com'
          ...
          if next_page_url:
            if 'javascript' not in next_page_url:
                  next_page_url = 'http://investors.lennar.com' + next_page_url
    ...
    def parse_details(self, response):
        ...
        name_regex = r'(Forward(.|\s*)Looking\s*Statements)(.|\s)*|(\bABOUT\s*Lennar\s*(?!\'s)\b)(.|\s)*|(\bABOUT.Lennar\s*(?!\'s)\b)(.|\s)*' #|(\bABOUT\s*L\s*BRANDS\b)(.|\s)*'
