...
### Xylem Inc. 2|2
### 2nd spider normal Press Releases
### normal get request 
### goes back to 20111101


class QuotesInfiniteScrollSpider(scrapy.Spider):
    name = "Xylem_II_5389600ARV002"
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/Xylem_II_5389600ARV002/',
        }
    ...
    def start_requests(self):  # follow drop down menue for different years
         years = list(range(1, 7)) # fill in years which should be scraped, always last yeat +1 as upper bound will not be element of the list
         #del years[0]  # delets first element "NULL" from list of years
         for year in years:
             aux_url = 'https://www.xylem.com/en-us/about-xylem/newsroom/press-releases?page={}&pagesize=96'
    ...
    def parse_next(self, response):
        auxs = response.xpath('//div[@class="result-details"]')
        for aux in auxs:
            item = SwisscomIvCrawlerItem()
            item['PUBSTRING'] = aux.xpath('.//div[@class="press-release-date pure-u-1 pure-u-sm-1-4"]/text()').extract_first()
...
#
    ...
    def parse_details(self, response):
        ...
        name_regex_2=r'(\bAbout\s*Xylem)(.|\s)*|(\bAbout.Xylem\b)(.|\s)*|(\bABOUT.Xylem\b)(.|\s)*|(\bABOUT\s*.XYLEM\b)(.|\s)*'
        if '.pdf' in response.url.lower():
            ...
            item['DESCRIPTION'] = re.sub(name_regex,'' ," ".join(response.xpath('//div[contains(@class, "detail-content press-release-content")]//text()[not(ancestor::div[@class="box__right"] or self::style or self::script or  ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE)
            item['DESCRIPTION'] = re.sub(name_regex_2,'' , item['DESCRIPTION'])
            item['DOCLINK'] = response.url
            if not item['DESCRIPTION']:
                item['DESCRIPTION'] = re.sub(name_regex,'' ," ".join(response.xpath('//div[@class="article-deatils pressRelease"]//text()[not(ancestor::div[@class="box__right"] or self::style or self::script or  ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()))
                item['DESCRIPTION'] = re.sub(name_regex_2,'' , item['DESCRIPTION'])
            yield item
