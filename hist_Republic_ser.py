...
### 1 spider News Releases, IR releases web page not complete all releases also on news page
### normal get request back to 20000726
### Achtung, cannot exclude forward looking statements as it cuts out financial guidance from ERNAs

class QuotesInfiniteScrollSpider(scrapy.Spider):
    name = "Republic_ser_3483800ARV001"
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/Republic_ser_3483800ARV001/',
        }
    ...
    start_urls = ['http://media.republicservices.com/news-releases'] 

    
    def parse(self, response):  # follow drop down menue for different years
         years = list(range(0,400, 100)) # fill in years which should be scraped, always last yeat +1 as upper bound will not be element of the list
         #del years[0]  # delets first element "NULL" from list of years
         for year in years:
             aux_url = 'http://media.republicservices.com/news-releases?l=100&o={}'
    ...
    def parse_next(self, response):
        auxs = response.xpath('//div[@class="wd_newsfeed_releases"]/ul/li[@class="wd_item"]')
        for aux in auxs:
            ...
            #        }
            base_url = 'http://media.republicservices.com'
    ...
    def parse_details(self, response):
        ...
        ### Achtung, cannot exclude forward looking statements as it cuts out financial guidance from ERNAs
        name_regex = r'(Republic\s*Services,\s*Inc.\s*is\s*a\s*leading\s*provider)(.|\s)*|(\bAbout\s*Republic\s*Services\b)(.|\s)* | (\bAbout.Republic.Serivces\b)(.|\s)*'
        if '.pdf' in response.url.lower():
            ...
            item['DESCRIPTION'] =re.sub(r'(INFORMATION\s*REGARDING\s*FORWARD(.|\s*)LOOKING\s*STATEMENTS)(.|\s)*', '', re.sub(name_regex,'' ," ".join(response.xpath('//div[@class="wd_subtitle wd_language_left"][not(self::style or self::script or descendant::style or descendant::script)]//text()[not(ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])] | //div[@class="wd_body wd_news_body"]/*[not(self::style or self::script or descendant::style or descendant::script)]//text()[not(ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE))
            item['DOCLINK'] = response.url
