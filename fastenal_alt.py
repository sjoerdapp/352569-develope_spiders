...
### 1 spider IR News Releases, 
### Press room releases are not primary  but from other sources -> no spider
### normal get request potentially al news and all pdfs

class QuotesInfiniteScrollSpider(scrapy.Spider):
    name = "Fastenal_I_9440000ARV001"
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/Fastenal_I_9440000ARV001/',
        }
    ...
    start_urls = ['https://investor.fastenal.com'] 

    
    def parse(self, response):  # follow drop down menue for different years
         years = list(range(0, 25)) # fill in years which should be scraped, always last yeat +1 as upper bound will not be element of the list
         #del years[0]  # delets first element "NULL" from list of years
         for year in years:
             aux_url = 'https://investor.fastenal.com/press-releases?items_per_page=20&page={}'
    ...
    def parse_next(self, response):
        ...
        for aux in auxs:
            ...
            item['PUBSTRING'] = aux.xpath('.//div[contains(@class, "news-date")]/div[@class="field__item"]/text()').extract_first()
            item['HEADLINE']= aux.xpath('.//div[contains(@class, "news-title")]//a[2]/text()').extract_first()
            item['DOCLINK']= aux.xpath('.//div[contains(@class, "news-title")]//a[2]/@href').extract_first()
            #item = {
            ...
            #        }
            base_url = 'https://investor.fastenal.com'
            aux_url = aux.xpath('.//div[contains(@class, "news-title")]//a[2]/@href').extract_first()
    ...
    def parse_details(self, response):
        ...
        name_regex = r'(\bADDITIONAL\s*INFORMATION\b)(.|\s)*|(Forward(.|\s*)Looking\s*Statements)(.|\s)*|(\bAbout\s*Fastenal\b)(.|\s)*|(\bAbout.Fastenal\b)(.|\s)*'
