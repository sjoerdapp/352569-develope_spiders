...
class QuotessSpider(scrapy.Spider):
    name = 'HCA_HealtCare_5369600ARV001'
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/HCA_HealtCare_5369600ARV001/',
        }
    #custom_settings = {
    ...
    def start_requests(self):  # follow drop down menue for different years
         years = list(range(0, 41)) # fill in years which should be scraped, always last yeat +1 as upper bound will not be element of the list
         #del years[0]  # delets first element "NULL" from list of years
         for year in years:
             aux_url = 'https://investor.hcahealthcare.com/press-releases/all/all/all?page={}'
             year_url = [aux_url.format(year)][0]
             yield scrapy.Request(url=year_url, callback=self.parse_next)

    def parse_next(self, response):
          auxs = response.xpath('//div[contains(@class, "press_release")]//div[@class="view-content"]/div[@class="item-list"]//ul/li')
          for aux in auxs:
              ...
              #        }
              base_url = 'https://investor.hcahealthcare.com'
    ...
    def parse_details(self, response):
        ...
        name_regex_2=r'(\bAbout\s*HCA)(.|\s)*|(\bAbout.HCA\b)(.|\s)*|(\bABOUT.HCA\b)(.|\s)*|(\bABOUT\s*.HCA\b)(.|\s)*'
