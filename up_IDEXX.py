            ...
            for aux in auxs:
                ...
                if '.pdf' in aux_url.lower() or 'static-files' in aux_url.lower():
                  if aux_url.startswith('http'):
                      ...
                      item['DOCLINK'] = url
                      item['DESCRIPTION'] = ''
                      item['PUBSTRING'] = 'Januar 31 2020'
                      #if re.search(r'(\d{8})', url):   
                      #  item['PUBSTRING'] = 'FF' #re.search(r'(\d{8})', url).group()
                      #  yield item
                      #else:
                      #if re.search(r'([a-zA-Z]+.\d+..\d{4})', item['HEADLINE']):
                      #  if '2020' in url:#re.search(r'(\d{8})', url):
                      #    item['PUBSTRING'] = 'Januar 31 2020' #re.search(r'(\d{8})', url).group()
                      #    yield item
                      #  else:  
