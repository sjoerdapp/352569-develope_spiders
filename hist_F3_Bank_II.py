...
### fifth Third Bankcorp 2|2
### 1 spider IR News Releases, 2nd spider Media Center News Reseases
### 2nd spider only little news and history up to 2017, however some news are not on the IR page
### one get all news are on first page...
### goes back to 20170105


class QuotesInfiniteScrollSpider(scrapy.Spider):
    name = "F3_Bank_II_1741000ARV002"
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/F3_Bank_II_1741000ARV002/',
        }
    ...
    start_urls = ['https://www.53.com/content/fifth-third/en/media-center/press-releases.html'] 

    
   
    def parse(self, response):
        auxs = response.xpath('//div[@class="isotope press-release-container"]/div[contains(@class, "c-gen-card__wrap")]')
        for aux in auxs:
            item = SwisscomIvCrawlerItem()
            item['PUBSTRING'] = aux.xpath('.//div[@class="c-gen-card__date"]/text()').extract_first()
            item['HEADLINE']= aux.xpath('.//div[@class="c-gen-card__title"]/a/text()').extract_first()
            item['DOCLINK']= aux.xpath('.//div[@class="c-gen-card__title"]/a/@href').extract_first()
            #item = {
            ...
            #        }
            base_url = 'https://www.53.com'
            aux_url = aux.xpath('.//div[@class="c-gen-card__title"]/a/@href').extract_first()
    ...
    def parse_details(self, response):
        ...
        if '.pdf' in response.url.lower():
            ...
            item['DESCRIPTION'] = re.sub(name_regex,'' ," ".join(response.xpath('//div[@class="c-article__content"]//text()[not(ancestor::div[@class="box__right"] or self::style or self::script or  ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE)
            item['DOCLINK'] = response.url
