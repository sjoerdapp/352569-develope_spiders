...
### Boston Properties Inc. 1|1
### 1st spider 
### normal get
### back to 19981029

class QuotesInfiniteScrollSpider(scrapy.Spider):
    name = "Bost_Prop_3199100ARV001"
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/Bost_Prop_3199100ARV001/',
        }
    ...
    start_urls = ['http://ir.bostonproperties.com/phoenix.zhtml?c=120176&p=irol-news_corp&newsReleasesBlurbHover01.3_rs=0&newsReleasesBlurbHover01.3_rc=10'] 

    
    def parse(self, response):  # follow drop down menue for different years
         years = list(range(11, 502, 10)) # fill in years which should be scraped, always last yeat +1 as upper bound will not be element of the list
         years.insert(0, 0)
         #del years[0]  # delets first element "NULL" from list of years
         for year in years:
             aux_url = 'http://ir.bostonproperties.com/phoenix.zhtml?c=120176&p=irol-news_corp&newsReleasesBlurbHover01.3_rs={}&newsReleasesBlurbHover01.3_rc=10'
    ...
    def parse_next(self, response):
        auxs = response.xpath('//table//tr[@class="oddbg" or @class="ccbnBgTblEven"]')
        for aux in auxs:
            item = SwisscomIvCrawlerItem()
            item['PUBSTRING'] = aux.xpath('./td/span/text()').extract_first() # cuts out the part berfore the date as well as the /n at the end of the string
            item['HEADLINE']= aux.xpath('./td/span/a/text()').extract_first()
            item['DOCLINK']= aux.xpath('./td/span/a/@href').extract_first()
            #item = {
            #        'PUBSTRING': aux.xpath('./td/span/text()').extract_first(),
            #        'HEADLINE': aux.xpath('./td/span/a/text()').extract_first(),
            #        'DOCLINK': aux.xpath('./td/span/a/@href').extract_first(),
            #        }
            base_url = 'http://ir.bostonproperties.com/'
            aux_url = aux.xpath('./td/span/a/@href').extract_first()
    ...
    def parse_details(self, response):
        ...
        name_regex = r'(Forward(.|\s*)Looking\s*Statements)(.|\s)*|(Boston\s*Properties\s*is\s*a\s*fully\s*integrated)(.|\s)*|(\bAbout\s*Boston\s*Properties\b)(.|\s)*|(\bAbout.Boston.Properties\b)(.|\s)*'
        if '.pdf' in response.url.lower() or 'external.file' in response.url.lower():
            ...
            item['DESCRIPTION'] = re.sub(name_regex,'' ," ".join(response.xpath('//table//tr[@class="ccbnBgTxt"]/td//text()[not(ancestor::div[@class="box__right"] or self::style or self::script or  ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE)
            item['DOCLINK'] = response.url
