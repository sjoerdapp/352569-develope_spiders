...
### fifth Third Bankcorp 1||2
### 1 spider IR News Releases, 2nd spider Media Center News Reseases
### normal get request and all pdfs

class QuotesInfiniteScrollSpider(scrapy.Spider):
    name = "F3_Bank_I_1741000ARV001"
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/F3_Bank_1741000ARV001/',
        }
    ...
    start_urls = ['https://ir.53.com/views/ajax?js=1&page=0%2C2&tid=All&view_name=bw_pr_custom&view_display_id=panel_pane_24&view_args=all%2Fall%2Fall&view_path=news-releases%2Fall%2Fall%2Fall&view_base_path=null&view_dom_id=1&pager_element=1'] 

    
    def parse(self, response):  # follow drop down menue for different years
         years = list(range(0, 16)) # fill in years which should be scraped, always last yeat +1 as upper bound will not be element of the list
         #del years[0]  # delets first element "NULL" from list of years
         for year in years:
             aux_url = 'https://ir.53.com/views/ajax?js=1&page=0%2C{}&tid=All&view_name=bw_pr_custom&view_display_id=panel_pane_24&view_args=all%2Fall%2Fall&view_path=news-releases%2Fall%2Fall%2Fall&view_base_path=null&view_dom_id=1&pager_element=1'
    ...
    def parse_next(self, response):
        ...
        cont = body['display']
        auxs = Selector(text=cont).xpath('//div[@class="item-list"]/ul/li[contains(@class, "views-row")]')
        for aux in auxs:
            item = SwisscomIvCrawlerItem()
            item['PUBSTRING'] = aux.xpath('.//span[@class="field-content"]/text()').extract_first()
            item['HEADLINE']= aux.xpath('.//div[@class="field-content"]/a/p/text()').extract_first()
            if not item['HEADLINE']:
                item['HEADLINE']= aux.xpath('.//div[@class="field-content"]/a/text()').extract_first()

            item['DOCLINK']= aux.xpath('.//div[@class="field-content"]/a/@href').extract_first()
            #item = {
            ...
            #        }
            base_url = 'https://ir.53.com'
            aux_url = aux.xpath('.//div[@class="field-content"]/a/@href').extract_first()
    ...
    def parse_details(self, response):
        ...
        name_regex = r'(Fifth\s*Third\s*Bancorp\s*is\s*a\s*diversified\s*financial\s*services\s*company)(.|\s)*|(Forward(.|\s*)Looking\s*Statements)(.|\s)*|(\bAbout\s*Fifth\s*Third\b)(.|\s)* | (\bAbout.Fifth.Third\b)(.|\s)*'
        if '.pdf' in response.url.lower():
            ...
            # xpath_I= '//div[contains(@class, "pressreleasesubheadline")]//text()[not(ancestor::div[@class="box__right"] or self::style or self::script or  ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]'
            item['DESCRIPTION'] = re.sub(name_regex,'' ," ".join(response.xpath('//div[contains(@class, "pressreleasesubheadline")]//text()[not(ancestor::div[@class="box__right"] or self::style or self::script or  ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]|//div[contains(@class,"pane-pr-body")]//div[@class="pane-content"]//text()[not(ancestor::div[@class="box__right"] or self::style or self::script or  ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE)
            item['DOCLINK'] = response.url
