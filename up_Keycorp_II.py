...
### UPDATES
### latest 20 news, seems that page is not updated anymore....

### KeyCorp 2||2
### 1 spider für IR All Reseases 2nd spider fpr Community Releases come with different request 'api.json?rss'
### back to 20181218

class QuotesInfiniteScrollSpider(scrapy.Spider):
    name = "KeyCorp_II_2535000ARV002"
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/KeyCorp_II_2535000ARV002/',
        }
    ...
    start_urls = ['https://api.rss2json.com/v1/api.json?rss_url=http%3A%2F%2F3blmedia.com%2FtheCSRfeed%2Forg%2F49735&callback=_cb_1550222935077356'] 

    
      
    def parse(self, response):
        body = response.body.decode("utf-8")
        data = json.loads(body.split('77356(')[1].split(');')[0])  ## cut out json from body
    ...
    def parse_details(self, response):
        ...
        name_regex = r"""(KeyCorp\s*'\s*s\s*(\(\s*NYSE\s*:\s*KEY\s*\)\s*)?roots\s*trace\s*back\s*190\s*years\s*to\s*Albany.)(.|\s)*|(Forward(.|\s*)Looking\s*Statements)(.|\s)*"""###|(\bAbout\s*KeyCorp\b)(.|\s)*| (\bAbout.KeyBank\b)(.|\s)*"""
        if '.pdf' in response.url.lower():
            ...
            item['DESCRIPTION'] = re.sub(name_regex,'' ," ".join(response.xpath('//div[@class="content"]//div[@id="fmr-body"]//div[@class="field-items"]//text()[not(ancestor::div[@class="box__right"] or self::style or self::script or  ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE)
            item['DOCLINK'] = response.url
