...
### normal get request 
### goes back to 20031110


class QuotesInfiniteScrollSpider(scrapy.Spider):
    name = "Synopsys_1136100ARV001"
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/Synopsys_1136100ARV001/',
        }
    ...
    def start_requests(self):  # follow drop down menue for different years
         ...
         for year in years:
             aux_url = 'https://news.synopsys.com/index.php?s=20295&l=50&o={}'
...
#
    ...
    def parse_details(self, response):
        ...
        name_regex = r'(certains*\statements\s*made\s*in\s*the\s*earnings\s*conference\s*call\s*are\s*Forward(.|\s*)Looking\s*Statements)(.|\s)*|(This\s*press\s*release\s*contains\s*Forward(.|\s*)Looking\s*Statements\s*within\s*the)(.|\s)*'
        name_regex_2=r'(\bAbout\s*Synopsys)(.|\s)*|(\bAbout.Synopsys\b)(.|\s)*|(\bABOUT.Synopsys\b)(.|\s)*|(\bABOUT\s*.SYNOPSYS.TWO\b)(.|\s)*'
