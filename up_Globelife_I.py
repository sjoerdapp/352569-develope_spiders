...
class QuotessSpider(scrapy.Spider):
    name = 'GlobeLife_I_2176400ARV001'
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/GlobeLife_I_2176400ARV001/',
        }
    #custom_settings = {
    ...
    def parse(self, response):
          auxs = response.xpath('//ul[@class="linklist"]/li')
          for aux in auxs[0:20]:
              item = SwisscomIvCrawlerItem()
              date_reg = r'.*\d{2}.\s*\d{4}'
              item['PUBSTRING'] = re.search(date_reg, aux.xpath('./a/text()').extract_first()).group(0) # cuts out the part berfore the date as well as the /n at the end of the string
    ...
    def parse_details(self, response):
        ...
        if '.pdf' in response.url.lower() or 'external.file' in response.url.lower():
            ...
            item['DESCRIPTION'] = re.sub(name_regex,'' ," ".join(response.xpath('//div[@class="container"]//text()[not(ancestor::h1 or ancestor::p[@class="relloc"] or ancestor::p[@class="reldate"])][not(ancestor::div[@class="box__right"] or self::style or self::script or  ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE)
