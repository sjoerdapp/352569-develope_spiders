...
### 2nd spider Newsroom
### normal get request 
### all news on one page
### back to 20140729

class QuotesInfiniteScrollSpider(scrapy.Spider):
    name = "AllegionPLC_II_5464100ARV002"
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/AllegionPLC_II_5464100ARV002/',
        }
    ...
    start_urls = ['https://www.allegion.com/corp/en/news-landing-page.html'] 
    ...
    def parse(self, response):
        auxs = response.xpath('//div[contains(@class, "news-box")]')
        for aux in auxs:
            item = SwisscomIvCrawlerItem()
            #item['PUBSTRING'] = aux.xpath('./th/text()[2]').extract_first()
            item['HEADLINE']= aux.xpath('./div[@class="box-header"]/p/text()').extract_first()
            item['DOCLINK']= aux.xpath('./div[@class="box-footer"]/a/@href').extract_first()
            #item = {
            ...
            #        }
            base_url = 'https://www.allegion.com'
    ...
    def parse_details(self, response):
        ...
        if '.pdf' in response.url.lower() or 'external.file' in response.url.lower():
            ...
            item['PUBSTRING'] = response.xpath('//div[@class="date"]/text()').extract_first()
            item['DESCRIPTION'] = re.sub(name_regex,'' ," ".join(response.xpath('//div[@class="content-container"]//div[contains(@class, "text")]//text()[not(ancestor::h1 or ancestor::div[@class="image"] or ancestor::div[@class="date"])][not(ancestor::div[@class="box__right"] or self::style or self::script or  ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE)
