...
class QuotessSpider(scrapy.Spider):
    name = 'Cabot_OG_2602400ARV001'
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/Cabot_OG_2602400ARV001',
        }
    #custom_settings = {
    ...
    #}
    start_urls = ['https://cabotoilgas.gcs-web.com/news?acfa0974_year%5Bvalue%5D=_none&op=Filter&acfa0974_widget_id=acfa0974&form_build_id=form-kfYetHhoQqnYneYBVWVkfYJrRQef3MHwLTTCkEUsJa0&form_id=widget_form_base']
    ...
    def parse(self, response):
          auxs = response.xpath('//tr[@class="ccbnOutline"]//tr[not(contains(@class, "Txt") or contains(@class, "Outline") or contains(@class, "Ttl"))]')
          for aux in auxs[0:20]:
              item = SwisscomIvCrawlerItem()
              item['PUBSTRING'] = aux.xpath('.//td[@nowrap="nowrap"]/span/div/text()').extract_first() # cuts out the part berfore the date as well as the /n at the end of the string
              ...
              #        }
              base_url = 'https://cabotoilgas.gcs-web.com'
    ...
    def parse_details(self, response):
        ...
        name_regex = r'(Forward(.|\s*)Looking\s*Statements)(.|\s)*|(Cabot\s*Oil\s*&\s*Gas\s*Corporation\s*.\s*headquartered\s*in\s*Houston\s*.\s*Texas\s*.\s*is)(.|\s)*|(\bABOUT\s*Cabot\b)(.|\s)*|(\bABOUT.Cabot\b)(.|\s)*' #|(\bABOUT\s*L\s*BRANDS\b)(.|\s)*'
