...
### 1 spider IR Press Releases, 2nd spider News and Media
### normal get request 
### need to follow pagination links
### back to 20070215

class QuotesInfiniteScrollSpider(scrapy.Spider):
    name = 'Celanese_I_9900286ARV001'
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/Celanese_I_9900286ARV001/',
        }
    ...
    def start_requests(self):  # follow drop down menue for different years
         years = list(range(2007, 2020)) # fill in years which should be scraped, always last yeat +1 as upper bound will not be element of the list
         #del years[0]  # delets first element "NULL" from list of years
         for year in years:
             aux_url = 'https://investors.celanese.com/websites/celanese/English/3041/press-release.html?newsYear={}'
    ...
    def parse_next(self, response):
        auxs = response.xpath('//a[@class="newslist"]')
        for aux in auxs:
            item = SwisscomIvCrawlerItem()
            item['PUBSTRING'] = aux.xpath('.//span[@class="tile__title newslist__date"]/text()').extract_first().split('T')[0]
    ...
    def parse_details(self, response):
        ...
        if '.pdf' in response.url.lower():
            ...
            item['DESCRIPTION'] = re.sub(name_regex,'' ," ".join(response.xpath('//div[@class="news-content-wrapper"]//text()[not(ancestor::div[@class="box__right"] or self::style or self::script or  ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE)
