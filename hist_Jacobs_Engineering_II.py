...
class QuotessSpider(scrapy.Spider):
    ...
    #}
    start_urls = ['https://www.jacobs.com']

    def parse(self, response):  # follow drop down menue for different years
         years = list(range(0, 146)) # fill in years which should be scraped, always last yeat +1 as upper bound will not be element of the list
    ...
    def parse_next(self, response):
          auxs = response.xpath('//div[contains(@class, "articles")]//div [@class="views-row"]')
          for aux in auxs:
              item = SwisscomIvCrawlerItem()
              item['PUBSTRING'] = aux.xpath('.//div[@class="node__submitted"]/text()').extract_first() # cuts out the part berfore the date as well as the /n at the end of the string
              item['HEADLINE']= aux.xpath('.//h2/span/text()').extract_first()
              item['DOCLINK']= aux.xpath('.//a[@class="article-wrap-link"]/@href').extract_first()
              #item = {
