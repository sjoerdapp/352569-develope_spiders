...
### normal get request 
### goes back to 20000727


class QuotesInfiniteScrollSpider(scrapy.Spider):
    name = "MGMResortsInternational_9900198ARV001"
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/MGMResortsInternational_9900198ARV001/',
        }
    ...
    def start_requests(self):  # follow drop down menue for different years
         ...
         for year in years:
             aux_url = 'https://investors.mgmresorts.com/investors/news-releases/{}/default.aspx'
    ...
    def parse_next(self, response):
        ...
        for aux in auxs:
            ...
            #        }
            base_url = 'https://investors.mgmresorts.com'
...
#
    ...
    def parse_details(self, response):
        ...
        name_regex = r'(Forward(.|\s*)Looking\s*Statements)(.|\s)*|(MGM\s*MIRAGE(\s*\(\s*NYSE\s*:\s*MGG\s*\)\s*)?.\s*headquartered\s*in\s*Las\s*Vegas.\s*Nevada.\s*is\s*one\s*of)(.|\s)*'
        name_regex_2=r'(\bAbout\s*MGM)(.|\s)*|(\bAbout.MGM\b)(.|\s)*|(\bABOUT.MGM\b)(.|\s)*|(\bABOUT\s*.MGM\b)(.|\s)*'
