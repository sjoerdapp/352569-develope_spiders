...
### Harley Davidson Inc. 2|2
### 2nd spider News Stories
### normal get with json
### back to 20170711


class QuotessSpider(scrapy.Spider):
    name = 'HarleyDavidson_II_2088100ARV002'
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/HarleyDavidson_II_2088100ARV002/',
        }
    #custom_settings = {
    ...
    #}
    start_urls = ['https://www.harley-davidson.com/content/h-d/us/en/about-us/hd-news/jcr:content.newslist.general.json?currentPage=1&start=0&pageSize=104']
    ...
    def parse(self, response):
          ...
          for aux in body['data']['articles'][0:20]:
              item = SwisscomIvCrawlerItem()
              item['PUBSTRING'] = aux['bylineDate'] # cuts out the part berfore the date as well as the /n at the end of the string
              item['HEADLINE']= aux['headline']
              item['DOCLINK']= aux['pageLinkUrl']
              #item = {
              #        'PUBSTRING': aux.xpath('./p[@class="news-card-date"]//text()').extract()[1],
              #        'HEADLINE': aux.xpath('.//h3[@class="news-card-title"]/a//text()').extract_first(),
              #        'DOCLINK': aux.xpath('.//h3[@class="news-card-title"]/a/@href').extract_first(),
              #        }
              base_url = 'https://www.harley-davidson.com'
    ...
    def parse_details(self, response):
        ...
        if '.pdf' in response.url.lower() or 'external.file' in response.url.lower():
            ...
            item['DESCRIPTION'] = re.sub(name_regex,'' ," ".join(response.xpath('//div[@class="long-form-artical"]//text()[not(ancestor::div[@class="box__right"] or self::style or self::script or ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE)
