...
class QuotessSpider(scrapy.Spider):
    name = 'CDW_Corp_9900326ARV001'
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/CDW_Corp_9900326ARV001/',
        }
    #custom_settings = {
    ...
    def start_requests(self):  # follow drop down menue for different years
         ...
         for year in years:
             aux_url = 'https://investor.cdw.com/press-releases?items_per_page=20&page={}'
    ...
    def parse_next(self, response):
          ...
          for aux in auxs:
              ...
              item['DOCLINK']= aux.xpath('.//div[contains(@class, "news-title")]/div/a[not(contains(@class, "toggle"))]/@href').extract_first()
              #item = {
              #        'PUBSTRING': aux.xpath('./p[@class="news-card-date"]//text()').extract()[1],
              #        'HEADLINE': aux.xpath('.//h3[@class="news-card-title"]/a//text()').extract_first(),
              #        'DOCLINK': aux.xpath('.//h3[@class="news-card-title"]/a/@href').extract_first(),
              #        }
              base_url = 'https://investor.cdw.com'
    ...
    def parse_details(self, response):
        ...
        name_regex_2= r'(\bAbout\s*CDW)(.|\s)*|(\bAbout.CDW\b)(.|\s)*|(\bAbout. CDW\b)(.|\s)*|(\bABOUT\s*CDW\b)(.|\s)*|(\bABOUT\s*cdw)(.|\s)*'
