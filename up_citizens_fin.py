...
### 1 spider Investors Newsroom
### normal get request 
### data goes back to 2010 even filter only goes to 2013
### back to 20100104

class QuotesInfiniteScrollSpider(scrapy.Spider):
    name = "Citizens_Fin_9900068ARV001"
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/Citizens_Fin_9900068ARV001/',
        }
    ...
    start_urls = ['https://investor.citizensbank.com/about-us/newsroom'] 
    ...
    def parse(self, response):  # follow drop down menue for different years
         ...
         for year in years:
             aux_url = 'https://investor.citizensbank.com/about-us/newsroom/latest-news/{}.aspx'
    ...
    def parse_next(self, response):
        auxs = response.xpath('//div[contains(@class, "rp-content")]/div[@class="rp-container"]/div')
        if len(auxs) > 20:
            auxs = auxs[0:20]

        for aux in auxs:
            item = SwisscomIvCrawlerItem()
            item['PUBSTRING'] = aux.xpath('./div/p[1]/text()').extract_first()
            item['HEADLINE']= aux.xpath('./div[2]/p/a/text()').extract_first()
            #if not re.search('[a-zA-Z]', item['HEADLINE']):
            #    item['HEADLINE']= "".join(aux.xpath('.//h2/a/text()').extract())
            item['DOCLINK']= aux.xpath('./div[2]/p/a/@href').extract_first()
            #item = {
            ...
            #        }
            base_url = 'https://investor.citizensbank.com'
            aux_url = aux.xpath('./div[2]/p/a/@href').extract_first()
    ...
    def parse_details(self, response):
        ...
        name_regex = r'(Forward(.|\s*)Looking\s*Statements)(.|\s)*|(\bAbout\s*Citizens\s*Financial\b)(.|\s)* | (\bAbout.Citizens.Financial\b)(.|\s)*'
        if '.pdf' in response.url.lower():
            ...
            if not re.search('[a-zA-Z]', item['HEADLINE']):
                item['HEADLINE'] = " ".join(response.xpath('//h2//text()').extract())
            ...
            item['DESCRIPTION'] = re.sub(name_regex,'' ," ".join(response.xpath('//div[contains( @class, "main-content")]//text()[not(ancestor::h2 or ancestor::div[@class="box__right"] or self::style or self::script or  ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE)
            item['DOCLINK'] = response.url
