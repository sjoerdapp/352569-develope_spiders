...
### normal get request 
### goes back to 20040105


class QuotesInfiniteScrollSpider(scrapy.Spider):
    name = "CadenceDesign_9245000ARV001"
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/CadenceDesign_9245000ARV001/',
        }
    ...
    def parse_next(self, response):
        auxs = response.xpath('//table[1]//tr[@class="search-result-entry"]')
        for aux in auxs:
            item = SwisscomIvCrawlerItem()
            item['PUBSTRING'] = aux.xpath('.//p[@class="publishdate"]/text()').extract_first().split(':')[1]
            item['HEADLINE']= aux.xpath('.//h5/a/text()').extract_first()
            item['DOCLINK']= aux.xpath('.//h5/a/@href').extract_first()
            #item = {
...
#
    ...
    def parse_details(self, response):
        ...
        if '.pdf' in response.url.lower():
            ...
            item['DESCRIPTION'] = re.sub(name_regex,'' ," ".join(response.xpath('//div[@class="next-gen-col-wrapper next-gen-col-wrapper-1"]//text()[not(ancestor::div[@class="box__right"] or self::style or self::script or  ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE)
