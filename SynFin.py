...
### 1 spider Newsroom
### classic get request
### goes back to 20130111

class QuotesInfiniteScrollSpider(scrapy.Spider):
    name = "SynFin_9900053ARV001"
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/SynFin_9900053ARV001/',
        }
    ...
    def parse(self, response):  # follow drop down menue for different years
         years = list(range(1, 1111, 10)) # fill in years which should be scraped, always last yeat +1 as upper bound will not be element of the list
    ...
    def parse_next(self, response):
        ...
        for aux in auxs:
            ...
            aux_url= aux.xpath('./h4[@class="media-heading"]/a/@href').extract_first()
    ...
    def parse_details(self, response):
        ...
        item['DESCRIPTION'] = re.sub(r'(\bAbout\s*Motorola\s*Solutions\b)(.|\s)* | (\bAbout.Motorola.Solutions\b)(.|\s)*','' ," ".join(response.xpath('//div[@id="content-details"]/div//text()').extract()))
