...
class QuotessSpider(scrapy.Spider):
    ...
    #}
    start_urls = ['https://www.jmsmucker.com/company-news/brand-news-releases']
    ...
    def parse_details(self, response):
        ...
        item['DESCRIPTION'] = " ".join(response.xpath('//div[@class="content-container"]//div[@class="content"]//text()[not(ancestor::div[@class="back-to-top-container"])][not(ancestor::h4 or ancestor::div[@class="no-print"])][not(ancestor::div[@class="box__right"] or self::style or self::script or  ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract())
        ...
        #    yield item
        #else:
        #    item['DESCRIPTION'] = response.xpath('//div[@class="content"]//text()').extract()
        #    #[not(ancestor::h4)][not(ancestor::div[@class="box__right"] or self::style or self::script or  ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]
        #    item['DOCLINK'] = response.url
