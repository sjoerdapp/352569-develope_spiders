...
### 2 spider Press Releases Awards and Recognitions
...
class QuotessSpider(scrapy.Spider):
    name = 'Hormel_Foo_II_2093200ARV002'
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/Hormel_Foo_II_2093200ARV002/',
        }
    #custom_settings = {
    ...
    #}
    start_urls = ['https://www.hormelfoods.com/newsroom/press-releases/category/awards-recognition/']
    ...
    def parse(self, response):  # follow drop down menue for different years
          ...
          for aux in auxss:
              item = SwisscomIvCrawlerItem()
              item['PUBSTRING'] = aux.xpath('./div[contains(@class, "prDateCol")]//div[@class="irwPRDate"]/text()').extract_first() # cuts out the part berfore the date as well as the /n at the end of the string
              item['HEADLINE']= " ".join(aux.xpath('.//a[@class="c-card__title"]//text()').extract())
          ...
          years = list(range(2, 3)) # fill in years which should be scraped, always last yeat +1 as upper bound will not be element of the list
          #del years[0]  # delets first element "NULL" from list of years
          for year in years:
              aux_url = 'https://www.hormelfoods.com/wp-json/wp/v2/posts?orderby=date&order=desc&per_page=20&exclude=28453%2C26065%2C26004&categories=26&news_type=14&content=1&page={}'
    ...
    def parse_details(self, response):
        ...
        name_regex = r'xxx'#(Forward(.|\s*)Looking\s*Statements)(.|\s)*|(\bABOUT.Hormel.Foods)(.|\s)*|(\bABOUT\s*Hormel\s*Foods)(.|\s)*'
        ...
        if '.pdf' in response.url.lower() or 'external.file' in response.url.lower():
            ...
            item['DESCRIPTION'] = re.sub(name_regex,'' ," ".join(response.xpath('//div[@class="content-block__rich-text"]//text()[not(ancestor::img)][not(ancestor::div[@class="box__right"] or self::style or self::script or  ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE)
            item['DOCLINK'] = response.url
