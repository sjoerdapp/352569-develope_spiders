...
class QuotesInfiniteScrollSpider(scrapy.Spider):
    name = "Abiomed_2001800ARV001"
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/Abiomed_2001800ARV001/',
        }
    ...
    start_urls = ['http://investors.abiomed.com/all-press-releases'] 
    ...
    def parse(self, response):
        auxs = response.xpath('//table/*[not(self::thead or descendant::thead)]//tr')[0:20]
        for aux in auxs:
            item = SwisscomIvCrawlerItem()
            item['PUBSTRING'] = aux.xpath('./td[contains(@class, "news-date")]//time/text()').extract_first()
            ...
            #        }
            base_url = 'http://investors.abiomed.com'
    ...
    def parse_details(self, response):
        ...
        if '.pdf' in response.url.lower():
            ...
            item['DESCRIPTION'] = re.sub(r'(The\s*ABIOMED\s*logo,\s*ABIOMED)(.|\s)*|(\bAbout\s*Abiomed\b)(.|\s)* | (\bAbout.Abiomed\b)(.|\s)*','' ," ".join(response.xpath('//div [@class="node__content"]/*[not(self::style or self::script or descendant::style or descendant::script)]//text()[not(ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE)
            item['DOCLINK'] = response.url
