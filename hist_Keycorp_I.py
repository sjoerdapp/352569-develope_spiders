...
### KeyCorp 1|2
### 1 spider IR Press releases, 2nd spider Community releases -> only very short history
### normal get request 
### goes back to 20090109


class QuotesInfiniteScrollSpider(scrapy.Spider):
    name = "KeyCorp_2535000ARV001"
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/KeyCorp_2535000ARV001/',
        }
    ...
    start_urls = ['http://investor.key.com/News/100334/NewsData?pageIndex=1'] 

    
    def parse(self, response):  # follow drop down menue for different years
         years = list(range(0, 91)) # fill in years which should be scraped, always last yeat +1 as upper bound will not be element of the list
         #del years[0]  # delets first element "NULL" from list of years
         for year in years:
             aux_url = 'http://investor.key.com/News/100334/NewsData?pageIndex={}'
    ...
    def parse_next(self, response):
        ...
        for aux in auxs:
            ...
            #        }
            base_url = 'http://investor.key.com'
    ...
    def parse_details(self, response):
        ...
        name_regex = r"""(KeyCorp\s*'\s*s\s*(\(\s*NYSE\s*:\s*KEY\s*\)\s*)?roots\s*trace\s*back\s*190\s*years\s*to\s*Albany.)(.|\s)*|(Forward(.|\s*)Looking\s*Statements)(.|\s)*|(KeyCorp\s*was\s*organized\s*more\s*than\s*160\s*years\s*ago\s*and\s*is\s*headquartered\s*in\s*Cleveland,)(.|\s)* """
        if '.pdf' in response.url.lower():
            ...
            if not item['DESCRIPTION']:
                item['DESCRIPTION'] = re.sub(name_regex,'' ," ".join(response.xpath('//div[contains(@class, "PageBody")]//text()[not(ancestor::div[@class="box__right"] or self::style or self::script or  ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE)
