...
### VeriSign Inc 1|1
### has two pages Newsroom and IR press releases but investor press releases has the same news and better history as Newsroom
### spider for IR as investor press releases has the same news and better history as Newsroom
### ACHTUNG !! 19 quaterly results from 2002 to 2005 have empty pdfs
### goes back to 20010124


class QuotesInfiniteScrollSpider(scrapy.Spider):
    name = "Verisign_1594000ARV001"
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/Verisign_1594000ARV001/',
        }
    ...
    start_urls = ['https://investor.verisign.com/events-and-presentations/news-releases?field_nir_news_date_value%5Bmin%5D=&items_per_page=50&page=1'] 

    
    def parse(self, response):  # follow drop down menue for different years
         years = list(range(0, 7)) # fill in years which should be scraped, always last yeat +1 as upper bound will not be element of the list
         #del years[0]  # delets first element "NULL" from list of years
         for year in years:
             aux_url = 'https://investor.verisign.com/events-and-presentations/news-releases?field_nir_news_date_value%5Bmin%5D=&items_per_page=50&page={}'
    ...
    def parse_next(self, response):
        ...
        for aux in auxs:
            ...
            item['PUBSTRING'] = aux.xpath('./td/time/text()').extract_first()
            ...
            #item = {
            #        'PUBSTRING': aux.xpath('./td/time/text()').extract_first(),
            #        'HEADLINE': aux.xpath('./td[contains(@class, "news-title")]/a/text()').extract_first(),
            #        'DOCLINK': aux.xpath('./td[contains(@class, "news-title")]/a/@href').extract_first(),
            #        }
            base_url = 'https://investor.verisign.com'
            aux_url = aux.xpath('./td[contains(@class, "news-title")]/a/@href').extract_first()
            
            if '/static-files/' in aux_url.lower():
    ...
    def parse_details(self, response):
        ...
        name_regex = r"""(\bAbout\s*Verisign\b)(.|\s)* | (\bAbout.Verisign\b)(.|\s)*|(VRSNF)(.|\s)*|
                          (Forward(.|\s*)Looking\s*Statements)(.|\s)*|(VeriSign.\s*Inc.\s*\(\s*NASDAQ\s*:\s*VRSN\s*\)\s*is\s*the\s*trusted\s*provider)(.|\s)*"""
