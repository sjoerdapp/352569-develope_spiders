...
### UPDATE
### first update 2018 - 2019, after that update
### following actual year 
...
class QuotesInfiniteScrollSpider(scrapy.Spider):
    name = "Regions_fin_1620000ARV001"
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/Regions_fin_1620000ARV001/',
        }
    ...
    start_urls = ['https://ir.regions.com'] 
    ...
    def parse(self, response):  # follow drop down menue for different years
         ...
         for year in years:
             aux_url = 'https://ir.regions.com/news-and-events/press-releases/{}'
    ...
    def parse_next(self, response):
        ...
        for aux in auxs:
            ...
            item['PUBSTRING'] = aux.xpath('./td/span[@class="table-data"][not(descendant::a)]/text()').extract_first()
            item['HEADLINE']= aux.xpath('./td/span[@class="table-data"]/a/text()').extract_first()
            item['DOCLINK']= aux.xpath('./td/span[@class="table-data"]/a[not(descendant::span)]/@href').extract_first()
            #item = {
            ...
            #        }
            base_url = 'https://ir.regions.com'
            aux_url = aux.xpath('./td/span[@class="table-data"]/a[not(descendant::span)]/@href').extract_first()
        ...
        next_page_url = response.xpath('//div[@class="sitepagination"]/ul/li[@class="next"]/a/@href').extract_first()
        page_nr = response.xpath('//div[@class="sitepagination"]/ul/li[@class="pagerlink pageloaded"]/a/text()').extract_first()
        if '2' in page_nr:
            return
        if next_page_url:
            next_page_url = 'https://ir.regions.com' + next_page_url
    ...
    def parse_details(self, response):
        ...
        if '.pdf' in response.url.lower():
            ...
            item['DESCRIPTION'] = re.sub(r'(\bAbout\s*Regions\s*Financial\s*Corporation\b)(.|\s)*|(\bAbout.Regions.Financial.Corporation\b)(.|\s)*', ''," ".join(response.xpath('//div[@class="bw-release-story"]/*[not(self::style or self::script or descendant::style or descendant::script)]//text()[not(ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE)
            item['DOCLINK'] = response.url
