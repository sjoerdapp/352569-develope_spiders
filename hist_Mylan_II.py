...
### Mylan N.V. 2|2
### 1st spider Newsroom, 2nd spider IR Latest Releases
### two pages seem indentical for the younger history. IR goes back to 2002
### to be sure scrape both pages
### normal get request goes back until 20020505
### goes back

class QuotesInfiniteScrollSpider(scrapy.Spider):
    name = "Mylan_II_3104000ARV002"
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/Mylan_II_3104000ARV002/',
        }
    ...
    start_urls = ['http://investor.mylan.com'] 

    
    def parse(self, response):  # follow drop down menue for different years
         years = list(range(0, 120)) # fill in years which should be scraped, always last yeat +1 as upper bound will not be element of the list
         #del years[0]  # delets first element "NULL" from list of years
         for year in years:
             aux_url = 'http://investor.mylan.com/press-releases?page={}'
    ...
    def parse_next(self, response):
        ...
        for aux in auxs:
            ...
            #        }
            base_url = 'http://investor.mylan.com'
            aux_url = aux.xpath('./div[contains(@class, "news--headline")]/a/@href').extract_first()
    ...
    def parse_details(self, response):
        ...
        if '.pdf' in response.url.lower():
            ...
            item['DESCRIPTION'] = re.sub(r'(Forward(.|\s*)Looking\s*Statements)(.|\s)*|(Mylan\s*is\s*a\s*global\s*pharmaceutical\s*company\s*committed\s*to\s*setting)(.|\s)*|(\bAbout\s*Mylan\b)(.|\s)* | (\bAbout.Mylan\b)(.|\s)*','' ," ".join(response.xpath('//div[@class="xn-content"]/*[not(self::style or self::script or descendant::style or descendant::script)]//text()[not(ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE)
            item['DOCLINK'] = response.url
            if not item['DESCRIPTION']:
                item['DESCRIPTION'] = re.sub(r'(Forward(.|\s*)Looking\s*Statements)(.|\s)*|(Mylan\s*is\s*a\s*global\s*pharmaceutical\s*company\s*committed\s*to\s*setting)(.|\s)*|(\bAbout\s*Mylan\b)(.|\s)* | (\bAbout.Mylan\b)(.|\s)*','' ," ".join(response.xpath('//div[@class="node__content"]//text()[not(self::style or self::script or ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE)
                yield item
            else:               
                yield item
