...
class QuotessSpider(scrapy.Spider):
    name = 'Autozone_2900900ARV001'
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/Autozone_2900900ARV001/',
        }
    #custom_settings = {
    ...
    def start_requests(self):  # follow drop down menue for different years
         years = list(range(1998, 2021)) # fill in years which should be scraped, always last yeat +1 as upper bound will not be element of the list
         #del years[0]  # delets first element "NULL" from list of years
         for year in years:
             aux_url = 'https://investors.autozone.com/news-releases?field_nir_news_date_value%5Bmin%5D={}&items_per_page=50&page=0'
             year_url = [aux_url.format(year)][0]
             yield scrapy.Request(url=year_url, callback=self.parse_next)

    def parse_next(self, response):
          auxs = response.xpath('//div[@class="item-list"]/table//tr[not(ancestor::thead)]')
          for aux in auxs:
              ...
              #        }
              base_url = 'https://investors.autozone.com'
    ...
    def parse_details(self, response):
        ...
        name_regex_2=r'(\bAbout\s*AutoZone)(.|\s)*|(\bAbout.AutoZone)(.|\s)*|(\bABOUT.AutoZone)(.|\s)*|(\bABOUT\s*AUTOZONE)(.|\s)*'
