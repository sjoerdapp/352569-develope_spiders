...
class QuotessSpider(scrapy.Spider):
    name = 'Centene_I_4254000ARV001'
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/Centene_I_4254000ARV001/',
        }
    #custom_settings = {
    ...
    def start_requests(self):  # follow drop down menue for different years
         years = list(range(2000, 2021)) # fill in years which should be scraped, always last yeat +1 as upper bound will not be element of the list
         #del years[0]  # delets first element "NULL" from list of years
         for year in years:
             aux_url = 'https://investors.centene.com/news-releases?field_nir_news_date_value%5Bmin%5D={}#views-exposed-form-widget-news-widget-news-ul'
    ...
    def parse_next(self, response):
          ...
          for aux in auxs:
              ...
              item['HEADLINE']= aux.xpath('.//div[contains(@class, "news-title")]/div/a[not(contains(@class, "summary"))]/text()').extract_first()
              item['DOCLINK']= aux.xpath('.//div[contains(@class, "news-title")]/div/a[not(contains(@class, "summary"))]/@href').extract_first()
              #item = {
              #        'PUBSTRING': aux.xpath('./p[@class="news-card-date"]//text()').extract()[1],
              #        'HEADLINE': aux.xpath('.//h3[@class="news-card-title"]/a//text()').extract_first(),
              #        'DOCLINK': aux.xpath('.//h3[@class="news-card-title"]/a/@href').extract_first(),
              #        }
              base_url = 'https://investors.centene.com'
    ...
    def parse_details(self, response):
        ...
        name_regex_2=r'(\bAbout\s*Centene)(.|\s)*|(\bAbout.Centene\b)(.|\s)*|(\bABOUT.Centene\b)(.|\s)*|(\bABOUT\s*CENTENE\b)(.|\s)*'
