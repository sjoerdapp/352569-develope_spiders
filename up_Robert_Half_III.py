...
### UPDATES
### EARNAS in the latest 20 news, 

### Robert half International Inc. 3|3
### 3rd spider only Earnings releases
### normal get 
### back to 20110113


class QuotessSpider(scrapy.Spider):
    name = 'Robert_Half_III_4690000ARV003'
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/Robert_Half_III_4690000ARV003/',
        }
    ...
    def parse_details(self, response):
        ...
        if release_match:
            ...
            release_date = item['DESCRIPTION'].split('results on')[1].split('at')[0].split('day,')[1]
            releas_year = re.search(r'\d{4}', item['PUBSTRING']).group(0)
            title = item['HEADLINE']
            release_match = re.search(r'ANNOUNCES.SCHEDULE', item['HEADLINE'])
            ...
            item['HEADLINE']= 'Earnings Announcement'
            item['PUBSTRING'] = release_date + releas_year
            item['file_urls'] = [self.doc_mapping[year][quarter].get('er')]
            item['DOCLINK'] = self.doc_mapping[year][quarter].get('er')
