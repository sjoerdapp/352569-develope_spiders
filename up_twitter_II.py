...
class QuotessSpider(scrapy.Spider):
    ...
    #}
    start_urls = ['https://blog.twitter.com/']

    def parse(self, response):  # follow drop down menue for different years
         pages = list(range(1, 3)) # fill in years which should be scraped, always last yeat +1 as upper bound will not be element of the list
    ...
    def parse_next(self, response):
          ...
          for aux in auxs:
              ...
              item['DOCLINK']= aux.xpath('.//a[not(contains(@class, "bold-14"))]/@href').extract_first()
              #item = {
              ...
              aux_url =aux.xpath('.//a[not(contains(@class, "bold-14"))]/@href').extract_first()
    ...
    def parse_details(self, response):
        ...
        if not re.search('([a-z-A-Z]+.*){10}', item ['HEADLINE']):
