...
class QuotessSpider(scrapy.Spider):
    ...
    def parse_details(self, response):
        ...
        if '.pdf' in response.url.lower() or 'external.file' in response.url.lower():
            ...
            if 'copy' in body['data']['blocks'][1]['data']['contents'][1]['data'].keys():
              text_II = body['data']['blocks'][1]['data']['contents'][1]['data']['copy']
              text_II = re.sub(name_regex,'' ," ".join(Selector(text=text_II).xpath('//p/text()[not(ancestor::div[@class="box__right"] or self::style or self::script or  ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE)
              text_I_II= text_I + text_II
              item['DESCRIPTION'] = re.sub(name_regexx,'' ,text_I_II)
