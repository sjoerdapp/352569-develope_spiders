...
### UPDATES
### always update current year as it are very little news per year

### Ulta Beauty Inc 1||1
### 1 spider News Releases
### normal get request only very little releases ca 10 per year
### goes back to 20130104

class QuotesInfiniteScrollSpider(scrapy.Spider):
    name = "Ulta_b_9900091ARV001"
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/Ulta_b_9900091ARV001/',
        }
    ...
    start_urls = ['http://ir.ultabeauty.com/news-releases/default.aspx'] 

    
    def parse(self, response):  # follow drop down menue for different years
         years = list(range(2019, 2021)) # Achtung noch keine News 2019 fill in years which should be scraped, always last yeat +1 as upper bound will not be element of the list
         #del years[0]  # delets first element "NULL" from list of years
         for year in years:
             aux_url = 'http://ir.ultabeauty.com/news-releases/{}/default.aspx'
    ...
    def parse_next(self, response):
        auxs = response.xpath('//div[@class="ModuleContentContainer grid"]/div[not(@class="ModuleFooter" or @class="RssLinkBottomContainer" or @class="ModuleNotFound")]')
        for aux in auxs:
            item = SwisscomIvCrawlerItem()
            item['PUBSTRING'] = aux.xpath('.//span[@class="ModuleDate"]/span/text()').extract_first()
            item['HEADLINE']= aux.xpath('./div//span[@class="ModuleHeadline"]/text()').extract_first()
            item['DOCLINK']= aux.xpath('./div[@class="wrapInner"]/a/@href').extract_first()
            #item = {
            ...
            #        }
            base_url = 'http://ir.ultabeauty.com'
            aux_url = aux.xpath('./div[@class="wrapInner"]/a/@href').extract_first()
    ...
    def parse_details(self, response):
        ...
        if '.pdf' in response.url.lower():
            ...
            item['DESCRIPTION'] = re.sub(r'(\bAbout\s*Ulta\s*Beauty\b)(.|\s)* | (\bAbout.Ulta.Beauty\b)(.|\s)*','' ," ".join(response.xpath('//div[@class="ModuleBody"]/*[not(self::style or self::script or descendant::style or descendant::script)]//text()').extract()), flags=re.IGNORECASE)
            item['DOCLINK'] = response.url
