...
class QuotessSpider(scrapy.Spider):
    name = 'SP_Global_2117900ARV002'
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/SP_Global_2117900ARV002/',
        }
    #custom_settings = {
    ...
    def parse_next(self, response):
          ...
          for aux in auxs:
              ...
              #        }
              base_url = 'http://investor.spglobal.com/'
    ...
    def parse_details(self, response):
        ...
        name_regex = r'(This\s*(earnings\s*|press\s*)?release\s*may\s*contain\s*Forward(.|\s*)Looking\s*Statements)(.|\s)*|For more information(.|\s)*|About S.p dow jones(.|\s)*|Forward.Looking Statements(.|\s)*|About S.p global(.|\s)*|About The McGraw(.|\s)*' #|(\bABOUT\s*L\s*BRANDS\b)(.|\s)*'
        ...
        if '.pdf' in response.url.lower() or 'external.file' in response.url.lower():
            ...
            item['DESCRIPTION'] = re.sub(name_regex,'' ," ".join(response.xpath('//h2[@class="irwFilePageH2"]/text() | //div[@class="irwFilePageBody"]//text()[not(ancestor::div[@class="box__right"] or self::style or self::script or  ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE)
            item['DOCLINK'] = response.url
