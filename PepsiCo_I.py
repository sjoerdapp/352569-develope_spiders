...
### Pepesi Company 1|2
### 1st spider Quaterly Earnings Announcements, 2nd spiderPress Releases
### normal get 
### back to 2015


class QuotessSpider(scrapy.Spider):
    name = 'PepsiCo_I_2138400ARV003'
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/PepsiCo_I_2138400ARV003/',
        }
    #custom_settings = {
    ...
    #}
    start_urls = ['https://www.pepsico.com/investors/financial-information/quarterly-earnings']
    ...
    def parse(self, response):
          auxs = response.xpath('//div[@class="card accordion__item"]')[0:6]
          for aux in auxs:
              ...
              item['HEADLINE']= aux.xpath('.//button/text()').extract_first()
              item['DOCLINK']= aux.xpath('.//div[@class="card-body"]/a[span[contains(text(), "Webcast")]]/@href').extract_first()
              ...
              pdf_url = aux.xpath('.//div[@class="card-body"]/a[span[contains(text(), "Press Release")]]/@href').extract_first()
              item['file_urls'] = [base_url + pdf_url]
              ...
              if '.pdf' in aux_url.lower() or 'static-files' in aux_url.lower():
                ...
                if aux_url.startswith('http'):
                    url= aux_url
                    request = SplashRequest(url=url, splash_headers={'Authorization': basic_auth_header('535209af07354fbbb4110611b27f7504', '')}, args={'wait': 0.5, 'timeout':25} ,meta={'dont_proxy': True,}, callback=self.parse_details)
    ...
    def parse_details(self, response):
        ...
        item['PUBSTRING'] = response.xpath('//span[@class="css-8z37q6-InfoBar"]/text()').extract_first()
