...
### classic get request with xpath, go through pages, 50 news per page
### 3 errors due to missing links, 1 FEHLER due to site without text
### back to 20031201 


class QuotessSpider(scrapy.Spider):
    name = 'CenturyLink_II_2042100ARV002'
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/CenturyLink_II_2042100ARV002/',
        }
    #custom_settings = {
    ...
    #}
    start_urls = ['http://news.centurylink.com']
    ...
    def parse_next(self, response):
          ...
          for aux in auxs:
              ...
              #        }
              base_url = 'http://news.centurylink.com'
    ...
    def parse_details(self, response):
        ...
        name_regex = r'(This\s*(earnings\s*|press\s*)?release\s*may\s*contain\s*Forward(.|\s*)Looking\s*Statements)(.|\s)*|(Forward(.|\s*)Looking\s*Statements)(.|\s)*|(\bABOUT\s*CenturyLink (?!\'s))(.|\s)*|(\bABOUT.CenturyLink (?!\'s))(.|\s)*'
