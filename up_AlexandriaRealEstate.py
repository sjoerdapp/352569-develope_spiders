...
class QuotessSpider(scrapy.Spider):
    name = 'AlexandriaRealEstate_3196500ARV001'
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/AlexandriaRealEstate_3196500ARV001/',
    ...
    def start_requests(self):
         ...
         for year in years:
             aux_url = 'http://investor.are.com/News/111908/NewsData?pageIndex={}'
             year_url = [aux_url.format(year)][0]
             yield scrapy.Request(url=year_url, callback=self.parse_next)

    def parse_next(self, response):
          auxs = response.xpath('//section[contains(@class, "irwLoadingdata")]')
          for aux in auxs:
              ...
              #        }
              base_url = 'http://investor.are.com'
    ...
    def parse_details(self, response):
        ...
        name_regex_2=r'(\bAbout\s*Alexandria)(.|\s)*|(\bAbout.Alexandria\b)(.|\s)*|(\bABOUT.Alexandria\b)(.|\s)*|(\bABOUT\s*.ALEXANDRIA\b)(.|\s)*'
