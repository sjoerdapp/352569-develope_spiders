...
### Waters Coporation 1|1
### 1 spider using NEWS RELEASES -  Data comes in DOC request
### normal get request with xpath
### all data in one request
### history back to 19980427, 611 News'

class QuotesInfiniteScrollSpider(scrapy.Spider):
    name = "Waters_corp_3120800ARV001"
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/Waters_corp_3120800ARV001/',
        }
    ...
    start_urls = ['https://waterscorporation.gcs-web.com/news-releases'] 

    
    
    
    def parse(self, response):
        auxs = response.xpath('//div[@class="view-content"]/div[@class="item-list"]/table//tr[not(ancestor::thead)]')
        for aux in auxs[0:20]:
            item = SwisscomIvCrawlerItem()
            item['PUBSTRING'] = aux.xpath('./td/div[contains(@class, "news-date field")]/div/text()').extract_first()
            ...
            #        }
            base_url = 'https://waterscorporation.gcs-web.com'
            aux_url = item['DOCLINK']
            
            if 'a8f27d0e-3b34-43fb-a810-900a863213a1' in aux_url.lower():
    ...
    def parse_details(self, response):
        ...
        name_regex = r'(Waters\s*Corporation\s*\(NYSE:\s*WAT\),\s*the\s*world\'s\s*leading\s*specialty)(.|\s)*|(For\s*over\s*50\s*years,\s*Waters\s*Corporation)(.|\s)*|(CAUTIONARY\s*STATEMENT\b)(.|\s)*|(Forward(.|\s*)Looking\s*Statements)(.|\s)*|(\bAbout\s*Waters\s*Corporation\b)(.|\s)* | (\bAbout.Waters.Corporation\b)(.|\s)*'
