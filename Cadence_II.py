...
### 2 nd  spider missing ERNAS
### scrape missing ernas from financial information page.....
### not in the actual updates just used if an ERNA is missing
### normal get request 
### goes back to 2010


class QuotesInfiniteScrollSpider(scrapy.Spider):
    name = "CadenceDesign_II_9245000ARV002"
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/CadenceDesign_II_9245000ARV002/',
        }
    ...
    start_urls = ['https://www.cadence.com/content/cadence-www/global/en_US/home/company/investors/financial-information.html'] 

    
    #def start_requests(self):  # follow drop down menue for different years
    #     years = list(range(0, 1851, 50)) # fill in years which should be scraped, always last yeat +1 as upper bound will not be element of the list
    #     #del years[0]  # delets first element "NULL" from list of years
    #     for year in years:
    #         aux_url = 'https://www.cadence.com/content/cadence-www/global/en_US/home/company/newsroom/press-releases.html?refinementfilters=AEMTemplate%3a%2fapps%2fcadence-www%2ftemplates%2fpage_newsroom_detail&sortList=AEMPublished%3adescending&rowsPerPage=50&start={}'
    ...
    def parse(self, response):
        ...
        select = [response.xpath('//div[child::h2[contains(text(), "2014")]]//li/a[contains(text(), "Q4 Earnings Press Release")or contains(text(), "Q3 Earnings Press Release")or contains(text(), "Q1 Earnings Press Release")]'),
                  response.xpath('//div[child::h2[contains(text(), "2013")]]//li/a[contains(text(), "Q3 Earnings Press Release")]'),
                  response.xpath('//div[child::h2[contains(text(), "2012")]]//li/a[contains(text(), "Q3 Earnings Press Release")or contains(text(), "Q1 Earnings Press Release")]'),
                  response.xpath('//div[child::h2[contains(text(), "2011")]]//li/a[contains(text(), "Q2 Earnings Press Release")]'),
                  response.xpath('//div[child::h2[contains(text(), "2010")]]//li/a[contains(text(), "Q3 Earnings Press Release")]'),
                  ]
        for auxs in select:
            for aux in auxs:
                item = SwisscomIvCrawlerItem()
                #item['PUBSTRING'] = aux.xpath('.//p[@class="publishdate"]/text()').extract_first().split(':')[1]
                ...
                #        }
                base_url = 'https://www.cadence.com'
...
#
    ...
    def parse_details(self, response):
        ...
        item['PUBSTRING'] = re.search(r'\d{2}(.*)\d{4}', response.xpath('//div[@class="pr-location-date section"]/p/text()').extract_first()).group()
        name_regex = r'(Forward(.|\s*)Looking\s*Statements)(.|\s)*'
        name_regex_2=r'(\bAbout\s*Cadence)(.|\s)*|(\bAbout.Cadence\b)(.|\s)*|(\bABOUT.Cadence\b)(.|\s)*|(\bABOUT\s*.CADENCE\b)(.|\s)*'
        if '.pdf' in response.url.lower():
            ...
            item['DESCRIPTION'] = re.sub(name_regex,'' ," ".join(response.xpath('//div[@class="col-lg-12"]//div[@style="display: block;"]//text()[not(ancestor::div[@class="box__right"] or self::style or self::script or  ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE)
