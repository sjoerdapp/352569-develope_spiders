...
class QuotessSpider(scrapy.Spider):
    name = 'Pentair_II_5418500ARV002'
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/Pentair_II_5418500ARV002/',
        }
    #custom_settings = {
    ...
    #}
    start_urls = ['https://www.pentair.com/en/about-pentair/newsroom/news-releases.html']
    ...
    #     for year in years:
    #         aux_url = 'https://investors.pentair.com/investor-relations/news-releases/{}/default.aspx'
    #         year_url = [aux_url.format(year)][0]
    #         yield scrapy.Request(url=year_url, callback=self.parse_next)

    def parse(self, response):
          auxs = response.xpath('//div[@class="press-item"]')
          for aux in auxs:
              item = SwisscomIvCrawlerItem()
              item['PUBSTRING'] = aux.xpath('./div[@class="press-item-date"]/text()').extract_first() # cuts out the part berfore the date as well as the /n at the end of the string
              item['HEADLINE']= aux.xpath('.//a[@class="press-item-title"]/text()').extract_first()
              item['DOCLINK']= aux.xpath('.//a[@class="press-item-title"]/@href').extract_first()
              #item = {
              #        'PUBSTRING': aux.xpath('./p[@class="news-card-date"]//text()').extract()[1],
              #        'HEADLINE': aux.xpath('.//h3[@class="news-card-title"]/a//text()').extract_first(),
              #        'DOCLINK': aux.xpath('.//h3[@class="news-card-title"]/a/@href').extract_first(),
              #        }
              base_url = 'https://www.pentair.com'
