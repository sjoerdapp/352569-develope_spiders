...
### Hartford Financial Services Group Inc. 1||1
### 1 spider Newsroom News Reseases
### normal get request with payload
### goes back to 20070103


class QuotesInfiniteScrollSpider(scrapy.Spider):
    name = "Hartford_fin_3138600ARV001"
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/Hartford_fin_3138600ARV001/',
        }
    ...
    start_urls = ['https://newsroom.thehartford.com/news-releases'] 

    
    def parse(self, response):  # follow drop down menue for different years
         years = list(range(0, 99)) # fill in years which should be scraped, always last yeat +1 as upper bound will not be element of the list
         #del years[0]  # delets first element "NULL" from list of years
         for year in years:
             aux_url = 'https://newsroom.thehartford.com/views/ajax?js=1&page=0%2C{}&view_name=bw_press_release_Hartford_Custom&view_display_id=panel_pane_7&view_args=all%2Fall&view_path=news-releases&view_base_path=null&view_dom_id=1&pager_element=1'
    ...
    def parse_next(self, response):
        ...
        auxs = Selector(text=body['display']).xpath('//div[@class="view-content"]/div[@class="item-list"][1]/ul/li')
        for aux in auxs:
            item = SwisscomIvCrawlerItem()
            item['PUBSTRING'] = aux.xpath('.//span[contains(@class, "views-field-created")]/span[@class="field-content"]/text()').extract_first()
            item['HEADLINE']= aux.xpath('.//span[@class="field-content"]/a/h3/text()').extract_first()
            item['DOCLINK']= aux.xpath('.//div[contains(@class, "views-field-title")]/span[@class="field-content"]/a/@href').extract_first()
            #item = {
            ...
            #        }
            base_url = 'https://newsroom.thehartford.com'
            aux_url = aux.xpath('.//div[contains(@class, "views-field-title")]/span[@class="field-content"]/a/@href').extract_first()
    ...
    def parse_details(self, response):
        ...
        name_regex = r'(Forward(.|\s*)Looking\s*Statements)(.|\s)*|(\bAbout\s*The\s*Hartford\b)(.|\s)* | (\bAbout.The.Hartford\b)(.|\s)*'
        if '.pdf' in response.url.lower():
            ...
            item['DESCRIPTION'] = re.sub(name_regex,'' ," ".join(response.xpath('//div[contains(@class, "fieldpressreleasesubheadline")]//text()[not(ancestor::div[@class="box__right"] or self::style or self::script or  ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])] | //div[@class="panel-pane pane-pr-body"]/div[@class="pane-content"]//text()[not(ancestor::div[@class="box__right"] or self::style or self::script or  ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE)
            item['DOCLINK'] = response.url
