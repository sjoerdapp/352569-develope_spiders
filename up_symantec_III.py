...
### UPDATES
### Übergangsspider da page von spider 2 seit Mai inaktiv. 
### verkauf eines teils von symantec und anschliessender name change
### weiter beobachten

### Symantec Corporation 3|3
### 1st spider Investor NEWS RELEASES, 2nd spider Archive
### classic post
### back to 20090108
...
class QuotessSpider(scrapy.Spider):
    ...
    def start_requests(self):
        headers = {
            ...
            'Origin': 'https://investor.symantec.com',
            'Referer': 'https://investor.symantec.com/About/Investors/press-releases/default.aspx',
        ...
        for year in list(range(2020, 2021)):  # loop iterating over different pages of ajax request
            data["year"] = year
            s_url = 'https://investor.symantec.com/Services/PressReleaseService.svc/GetPressReleaseList'
    ...
    def parse_details(self, response):
        ...
        name_regex = r'(Forward(.|\s*)Looking\s*Statements:)(.|\s)*|(Forward(.|\s*)Looking\s*Statements:)(.|\s)*|(Forward(.|\s*)Looking\s*Statements\s*(?!\'s))(.|\s)*|(\bABOUT\s*Symantec\s*(?!\'s)\b)(.|\s)*|(\bABOUT.Symantec\s*(?!\'s)\b)(.|\s)*|'
