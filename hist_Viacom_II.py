...
class QuotessSpider(scrapy.Spider):
    name = 'Viacom_II_5003200ARV002'
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/Viacom_II_5003200ARV002/',
        }
    #custom_settings = {
    ...
    def parse(self, response):  # follow drop down menue for different years
         years = list(range(0, 2940, 15)) # fill in years which should be scraped, always last yeat +1 as upper bound will not be element of the list
         #del years[0]  # delets first element "NULL" from list of years
         for year in years:
             aux_url = 'https://www.viacbs.com/media-center?limit=15&offset={}'
             year_url = [aux_url.format(year)][0]
             yield scrapy.Request(url=year_url, callback=self.parse_next)

    def parse_next(self, response):
          auxs = response.xpath('//ul[@class="content-wrapper"]/li')
          for aux in auxs:
              ...
              item['DOCLINK']= aux.xpath('.//div [@class="buttons"]/a[1]/@href').extract_first()
              #item = {
              #        'PUBSTRING': aux.xpath('./p[@class="news-card-date"]//text()').extract()[1],
              #        'HEADLINE': aux.xpath('.//h3[@class="news-card-title"]/a//text()').extract_first(),
              #        'DOCLINK': aux.xpath('.//h3[@class="news-card-title"]/a/@href').extract_first(),
              #        }
              base_url = 'https://ir.viacom.com'
