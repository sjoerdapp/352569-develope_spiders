...
class QuotessSpider(scrapy.Spider):
    ...
    def parse(self, response):
          ...
          for aux in body['press_releases'][0:20]:
    ...
    def parse_details(self, response):
        ...
        name_regex_2=r'(\bAbout\s*L3Harris\b)(.|\s)* | (\bAbout.L3Harris\b)(.|\s)*|(\bABOUT.L3Harris\b)(.|\s)*|(\bABOUT\s*.L3HARRIS\b)(.|\s)*'
        if '.pdf' in response.url.lower() or 'external.file' in response.url.lower():
            ...
            if not item['DESCRIPTION']:
                item['DESCRIPTION'] = re.sub(name_regex,'' ," ".join(response.xpath('//div[@class="col-12 col-md-8"]/div[@class="mb-5 mb-md-0"]//text()[not(ancestor::div[@class="box__right"] or self::style or self::script or ancestor::style or ancestor::script or ancestor::p[@id="news-body-cta"] or ancestor::div[@id="bwbodyimg"])]').extract()), flags=re.IGNORECASE)
                item['DESCRIPTION'] = re.sub(name_regex_2,'' , item['DESCRIPTION'])
