...
### UPDATES
### first page, latest 10 news

### Wabtec Corp 1|2
### 1st spider Financial Press Releases, 2nd spider Newsroom
### normal get 
### back to 20110131


class QuotessSpider(scrapy.Spider):
    name = 'Wabtec_I_3091300ARV001'
    custom_settings = {
         'JOBDIR' : 'None',
         'FILES_STORE' : 's3://352569/Wabtec_I_3091300ARV001/',
        }
    #custom_settings = {
    ...
    def start_requests(self):  # follow drop down menue for different years
         years = list(range(0, 1))#13 # fill in years which should be scraped, always last yeat +1 as upper bound will not be element of the list
    ...
    def parse_next(self, response):
          auxs = response.xpath('//article[contains(@class, "widget-list")]')
          for aux in auxs:
              ...
              item['DOCLINK']= aux.xpath('.//div[contains(@class, "link-more")]/a/@href').extract_first()
              #item = {
              #        'PUBSTRING': aux.xpath('./p[@class="news-card-date"]//text()').extract()[1],
              #        'HEADLINE': aux.xpath('.//h3[@class="news-card-title"]/a//text()').extract_first(),
              #        'DOCLINK': aux.xpath('.//h3[@class="news-card-title"]/a/@href').extract_first(),
              #        }
              base_url = 'https://ir.wabteccorp.com'
